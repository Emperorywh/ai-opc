 ...
 * 必须长什么样、经纬度的合法区间。具体校验库、测试框架、文件布局、类型名与实现模式
 * 均不在此冻结，留给后续 TASK 自行决定。
 */
 ...
 * 必须先在此登记并补齐对应投影实现（统一投影 TASK），再产出引用该 CRS 的资产。
 */
...
export const KNOWN_DATA_VERSIONS = {
  /**
   * 初始契约版本。
   * 本版本对应骨架阶段：仅含契约、验证入口与代表夹具，尚无任何生产地理数据。
   */
  V1_0_0: '1.0.0',
} as const
 ...
 * 数字，避免大小写或 Unicode 归一化歧义。具体编码方案（如与 GB/T 2260 adcode 对齐）
 * 由省级边界 TASK 决定，本契约只冻结可机检的稳定性与唯一性外壳。
 */
 ...
 * 中国主图地理范围（SPEC §3.3）。
 *
 * 这是一个参考常量：地形资产应覆盖此范围，但「资产范围是否恰好等于该值」属于资产级校验
 * （生产高程资 产 TASK），本契约层只用它做文档与人工对照，不强制嵌入校验，
 * 以免过早冻结尚未生产完成的资产边界。
 */
