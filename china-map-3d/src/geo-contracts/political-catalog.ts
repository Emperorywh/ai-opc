 ...
 * Three.js / 运行时渲染层 / 浏览器。运行时数据访问层（src/lib）、渲染层（src/three / src/scenes）、
 * 离线资产生产 / 校验层（scripts/verify-assets、scripts/political）与测试基线（tests/）都从此处导入
 * 同一份「SPEC §6 红线点名了哪些九段线段、哪些岛礁 / 附属岛屿、哪些争议区」——禁止在别处手写第二份
 * 等价清单（TASK-015 实现约束「唯一事实源来自 TASK-006」、TASK-006 实现约束「SPEC §6 点名必备项
 * 单一定义点」）。
 *
 ...
 *   领域真值，被资产深度校验（scripts/verify-assets/political-deep）与主图渲染准备
 *   （src/lib/political-features）共同复用，使「资产校验」与「运行时渲染准备」走同一份红线清单，
 *   不存在第二套九段线段数 / 台湾东侧段序号 / 点名岛礁名常量。
 *
 * 政治红线点名项（SPEC §6，逐条对应）：
 ...
 * 这里把 1..10 全部列为必备，资产深度校验与主图渲染准备据此断言「恰好 10 段且序号 1..10 全在」，
 * 任一缺段（尤其台湾东侧第 10 段）都会被确定性发现、阻断渲染准备（TASK-015 验证方式 2）。
 */
 ...
 * 单独命名导出，使深度校验、主图渲染准备与测试可以「按名引用 10」而非「按目录长度推断」，
 ...
 * 资产深度校验与主图渲染准备都对 segmentIndex===10 做**独立硬编码锚点**断言（不经
 * REQUIRED_NINE_DASH_SEGMENT_INDICES 间接得出），即便有人误改本目录常量，该锚点仍要求资产
 * 必须含此段，并在缺段时阻断主图渲染（TASK-015 验证方式 2「删除台湾东侧段 → 渲染准备失败」）。
 */
 ...
 * （TASK-006 验证方式 3），本目录不声称穷尽。资产深度校验与主图渲染准备都只断言「点名项在」。
 */
 ...
 *
 * 注意：TASK-015 主图渲染只呈现十段线与岛礁点位，不渲染争议区修正几何（争议区修正影响的是
 * 省级边界表达，由 TASK-014 省界层承载）。本常量在此导出供资产深度校验复用，主图渲染准备
 * 不对争议区做额外断言（争议区完整性的运行时把关由资产校验管线承担）。
 */
