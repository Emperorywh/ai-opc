 ...
 *
 * 领域红线（SPEC §5.3、§6、§8、§13）：
 * - DataV.GeoAtlas 与项目自补的九段线/岛礁/争议区数据一律标记为非官方审图，
 *   isOfficialSurvey=false 且 disclaimer 必填。
 * - Copernicus DEM 是公开科学数据集，但同样不是中国官方审图数据，正式发布前仍须走审图。
 * - originUrl 仅作为离线审计记录；运行时禁止据此发起任何网络请求（仓库全局约束：
...
export interface DataSourceDeclaration {
  ...
  readonly id: string
  /** 来源展示名称，例如 "Copernicus DEM GLO-30"、"阿里 DataV.GeoAtlas"。 */
  ...
  /**
   * 是否为官方审图数据。
   * 本项目当前所有来源（含 Copernicus DEM）均为非官方审图数据，发布前必须取得审图号。
   */
