/**
 * GPU 位移地形网格（渲染层，TASK-009）。
 *
 * 角色与依赖方向：
 * - 本组件属于渲染层（src/three），只负责「把一份已加载的 heightmap 纹理 + 一份受控配置装配成 GPU 位移
 *   地形 mesh」。它**只**依赖：配置层（resolveTerrainConfigOrThrow / TerrainRenderConfig）、坐标层
 *   （MAIN_MAP_WORLD_BOUNDS —— 米制世界包围盒的唯一源）、资产访问层（HeightmapTextureLoadResult）、
 *   本层着色器（TERRAIN_VERTEX_SHADER / TERRAIN_FRAGMENT_SHADER）、three / R3F。**禁止**自行读取
 *   GeoJSON、维护 hover、加载外网或做颜色分层（TASK-009 实现约束）——纹理与元数据由上层注入，
 *   颜色分层与 hover 由后续 TASK 在片元着色器 / 拾取层接管。
 *
...
export function ChinaTerrainMesh({ heightmap, config, rise = 1.0 }: ChinaTerrainMeshProps): ReactNode {
  ...
  // uniform 对象：依赖 meta / texture / config / rise / plane 尺寸；相关值变化时重建（R3F 会把新对象
  // 赋给 shaderMaterial.uniforms，渲染器每帧按引用上传，无需重编译着色器）。
  ...
  }, [texture, meta, config.exaggeration, rise])
