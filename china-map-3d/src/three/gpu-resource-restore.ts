 ...
 *   「纹理对象」打 needsUpdate 标记，纹理对象的 .image / .data 字段（CPU 源数据）由各资产 hook 在挂载期
 *   一次性加载、跨 context 丢失 / 恢复保持同一引用，Three.js 据此 .data 重新上传 GPU——绝不在本模块重新
 *   fetch / 重新解码（TASK-022 实现约束「GPU 资源恢复与 CPU 领域数据生命周期分离，不得因 context 丢失
 *   重复解码 32MB 高程数组或重复下载资产」）。
 *
 ...
 *   的生命周期耦合与重复所有权（TASK-022 实现约束「不存在重复资源所有权」）。
 *
 * 可测试性（TASK-022 验证方式 3「重复 context 丢失 / 恢复，预期监听器、Clock、CPU 高程缓存和 GPU 资源
 *   所有权数量稳定」）：
