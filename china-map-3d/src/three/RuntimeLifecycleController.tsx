/**
 * 大屏长时运行恢复生命周期的单一集中编排器（渲染层，TASK-022，SPEC §7.4）。
 *
 * 角色与依赖方向：
 ...
 *   并在阶段切换时回调上层（ChinaMapScreen）以驱动 DOM 诊断。
 ...
 *
 * 集中编排（TASK-022 实现约束「生命周期状态必须集中编排，场景层和 UI 只消费状态」「禁止每个组件各自
 *   监听 context 或 resize」）：
 ...
 *   丢失时暂停渲染、恢复时重建」；TASK-022 输出约束）：
 * - webglcontextlost：调用 event.preventDefault() 阻止默认不可恢复行为（不 preventDefault 则浏览器把 context
 *   标记为永久丢失，无法恢复——TASK-022 实现约束「阻止默认不可恢复行为」）；迁移到 context-lost；启动恢复
 *   超时定时器（contextRestoreTimeoutMs）。context-lost 期间 runtimeFrame.paused=true，各渲染层冻结视觉推进。
 ...
 *   重新解码 .r16，TASK-022 实现约束「GPU 资源恢复与 CPU 领域数据生命周期分离」）；重建成功 → running、
 ...
 *
 * resize 防抖（TASK-022 实现约束「resize 事件经过防抖后更新渲染器、相机和 overlay 所需尺寸」「resize 防抖
 *   不得吞掉最终尺寸」）：
 ...
 * - 卸载时清理防抖定时器与监听器，事件不再生效（TASK-022 验证方式 2「卸载后事件不再生效」）。
 *
 * 无重复监听 / 无重复 Clock / 无运行时几何分配（TASK-022 验证方式 3、实现约束「挂载 / 卸载、丢失 / 恢复期间
 *   不存在重复监听器、重复 Clock 或运行时几何分配循环」）：
 * - 监听器：context lost / restored 各注册一次（useEffect 依赖 [gl]，gl 稳定故挂载期一次）；回调经 applyEventRef
 *   间接调用，applyEvent 身份变化不重新注册监听器（只刷新 ref）。
