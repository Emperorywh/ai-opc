 ...
 *   heightmap 资产在运行时解码为「唯一一份」CPU 侧只读高程事实源，向省界贴地、标签定位、
 *   遮挡判断（SPEC §7.5）、海陆判断（SPEC §3.5）等所有领域消费者提供「真实米制海拔 + 双线性
 *   采样」的统一查询能力（SPEC §3.6 的 CPU 端 heightmap、§7.5 的遮挡判定共用同一份数据）。
 ...
 *   位移或法线（SPEC §7.1 红线、TASK-008 实现约束）。
 *
 ...
 *   共用同一高程事实源且解码语义相同（TASK-008 输出约束）。此处刻意「先解码四角再双线性米值」、
 *   不把编码四舍五入成整数（terrain-deep 的 bilinearSampleCode 会 round，那是资产统计口径），
 *   以保证运行时查询与 GPU 片元采样在亚米级一致。边缘像元（u=0/1、v=0/1）收敛到边界像元值，不外推。
 *
 * 共享生命周期（单份事实源，TASK-008 输出约束）：
 ...
 *
 * 内存策略（保持约 32MB 单份表示，TASK-008 实现约束）：
 ...
 *
 * 适用：测试夹具、或自带解码管线（如直接持有 Uint16Array）的调用方。meta 先经契约校验；
 * pixels.length 必须 = width*height，否则抛 raster-size-mismatch（加载期确定性失败）。
 * 不会复制 pixels：provider 直接持有调用方传入的数组引用，调用方在此之后不应再修改它。
 */
