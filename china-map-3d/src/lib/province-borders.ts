/**
 * 省级贴地边界的数据准备（领域层，TASK-014）。
 *
 * 角色与依赖方向：
 * - 本模块属于运行时访问层（src/lib），与 projection.ts / elevation.ts 同层。它把「34 省行政区几何
 *   （EPSG:4326 lon/lat）+ 共享双线性高程查询（ElevationProvider）+ 夸张系数 k + densify/epsilon 配置」
 *   确定性地变换为「按行政区分组、已投影到主图世界 (x,y,z)、沿真实地形连续起伏的线段端点列表」，
 *   供渲染层（src/three/ProvinceBorders）**只消费、不再计算**。本模块是 SPEC §3.6「贴地描边」的领域实现：
 *   先沿世界弧长 densify 到接近 heightmap 纹理分辨率，再对每个 densify 顶点查询共享高程、应用夸张系数、
 *   加最小 epsilon，使省界跨山脊/盆地/岛屿时连续起伏、不穿山、不悬空、不 z-fighting。
 * - 单向依赖：本模块只依赖契约层 src/geo-contracts（AdministrativeGeometryFeature / LonLatRing /
 *   AdministrativeGeometry 类型）、同层坐标权威 src/lib/projection（projectToWorld —— lon/lat→世界 x,z 的
 *   唯一入口）、同层高程权威 src/lib/elevation（ElevationProvider.queryAtWorld —— 世界点→真实米制海拔的
 *   唯一入口）。**禁止**依赖 React / R3F / Three.js / DOM / hover 状态 / src/config
 *   （TASK-014 实现约束「省界准备层单向依赖行政区、投影和高程查询，不得依赖 React hover 状态」
 *   「不得复制 heightmap 解码或投影公式」）。
 *
 * densify 间距（SPEC §3.6「沿弧长 densify，采样间距接近地形纹理分辨率」、TASK-014 实现约束「必须先 densify
 * 再对每个点采样高程，禁止只抬升 DataV 稀疏原始顶点后用直线跨越地形」）：
 * - DataV 原始省界折线顶点稀疏（一条省界可能只有几十个顶点跨越数百公里），若直接抬升这些稀疏顶点、
 *   用直线段连接，线段会从山脊的直线弦上穿过——在山脊两侧表现为「穿山」（线段从山体内部穿过）或
 *   「悬空」（线段从山体上方跨过）。故必须先沿弧长 densify：把每条边按「世界米制弧长 / 间距」切成
 *   足够多的子段，每个子段端点独立投影并查询高程，使折线逐顶点贴合地形。
 * - densify 在**世界米制空间**（投影后的世界 XZ 平面）做弧长均分，而非在经纬度空间：墨卡托在高纬放大，
 *   同样的经纬度跨度在北方对应更长的世界弧长；按世界弧长度量才能保证全境 densify 密度一致
 *   （SPEC §3.6「每 1–2 km 或约 plane 宽/4096 一个点」的「1–2 km」是世界米制，不是度）。
 * - 间距默认 = 主图世界宽度 / 4096 ≈ 1742 m（由 src/config/province-borders 提供，本模块作为入参接收），
 *   与 heightmap 纹素分辨率一一对应：每个 densify 子段约一个纹素宽，既不漏纹理级起伏，也不过密。
 *
 * 高程贴地（SPEC §3.6「采样 CPU 端解码的 heightmap 高度作为 y」、TASK-014 输出约束「所有边界点使用真实
 * 地形高度乘同一夸张系数并加最小可解释 epsilon」）：
 * - 每个 densify 顶点用共享 ElevationProvider.queryAtWorld(x, z) 查询真实米制海拔 h（与地形 vertex shader
 *   的 GPU 位移同一份高程事实源、同一双线性语义，TASK-008 输出约束），再令 world_y = h·k + epsilon。
 * - h·k 与 GPU vertex shader 内的 `displaced.y = h * uExaggeration`（src/three/terrain-shaders）以及
 *   src/config/terrain-config 的 displaceElevationToWorldY 是同一公式。此处内联 h·k 而非 import 该 helper：
 *   src/lib 不得反向依赖 src/config（配置层在访问层之上），且 h·k 是平凡乘法、属本层「应用夸张系数」
 *   的本职，注释标明镜像关系即可，不构成「复制公式」（被禁止的是复制 heightmap 解码 / 投影公式，本模块
 *   全部复用 projection / elevation 的唯一入口）。
 * - epsilon 把省界顶点放到地表外侧（上方），补偿 CPU/GPU 高程采样的亚米级浮点差异（z-fighting 的主防线
 *   是渲染层的 NDC 深度偏移，epsilon 是辅助）。epsilon 由 src/config 提供、本模块作为入参接收。
 *
 * 异常语义（TASK-014 输出约束「无效几何或高程查询失败时整条资产准备明确失败，不产生平地边界」）：
 * - 任何顶点投影失败（projectToWorld 返回失败——越出主图范围）、任何高程查询失败（queryAtWorld 返回
 *   失败——越出元数据范围 / provider 已释放）、输入 exaggeration 非有限 / spacing 非法 / features 为空 /
 *   全体退化（零线段）→ 抛 ProvinceBorderPrepError（带稳定 code），整条资产准备失败，绝不产出部分省份
 *   的平地边界。平地边界会把「省界未贴地」伪装成「成功」，违反 TASK 验收。
 * - 不为个别省份写魔法高度修补：所有省份走同一 densify + 查询 + h·k+epsilon 流水线，省份差异只来自各自
 *   几何与所在地形（TASK-014 实现约束「不得为个别省份在组件中写魔法高度修补」）。
 *
 * 分组与按行政区寻址（TASK-014 输出约束「draw call 优化不能破坏按行政区寻址的能力」、实现约束「后续
 * hover 仍须能确定性更新单一省份样式」）：
 * - 输出按 adminId 分组：每个 PreparedProvinceBorder 对应一个行政区，携带其全部环（外环 + 内环/洞 +
 *   多多边形的岛屿）densify 后的线段端点平铺数组。渲染层据此每个行政区建一个 LineSegments2（少数可审计
 *   分组，34 个 draw call），后续 hover（TASK-018）通过更新单一行政区的材质参数即可加亮加粗，不影响
 *   其他省份——本模块不在输出中混入 hover 状态（hover 由 TASK-018 交付）。
 */
 ...
 *
 * 把 spacing / epsilon 打包为对象而非散参：prepareProvinceBorders 的签名稳定、可读，且测试可直接构造字面量
 * 而不必依赖配置层。本接口不承载渲染参数（颜色 / 线宽 / 深度偏移属渲染层，不进领域层）。
 */
...
export interface PreparedProvinceBorder {
  /** 行政区稳定标识（CN- 前缀），渲染层据此分组、后续 hover 据此寻址。 */
 ...
 *
 * Polygon 与 MultiPolygon 在「环列表」层级统一：本模块对每个环（外环 / 内环 / 多多边形岛屿环）做相同的
 * densify + 贴地处理，不区分环的角色——内环（洞 / 飞地）与外环一样是省界的一部分（飞地边界），岛屿环
 * （海南、台湾等多多边形省份）同样完整保留（TASK-014 输出约束「支持多多边形、岛屿和内环」）。
 */
 ...
 *
 * 任一顶点 projectToWorld 失败（越出主图范围 [72°E,136°E]×[3°N,54°N]）→ 抛 projection-failed，整条准备失败。
 * 省界数据天然在境内，正常运行路径不触发；失败分支由自动化测试用越界点覆盖。
 */
 ...
 * 任一步失败（越出元数据范围 / provider 已释放 / 反投影失败）→ 抛 elevation-query-failed，整条准备失败，
 * 绝不回退到 y=0 平地（TASK-014 输出约束「不产生平地边界」）。
 *
 * h·k 与 GPU vertex shader 的 `displaced.y = h * uExaggeration`（src/three/terrain-shaders）、src/config
 * terrain-config 的 displaceElevationToWorldY 是同一公式；此处内联以避免 src/lib 反向依赖 src/config。
 */
 ...
 *
 * densify 策略（SPEC §3.6「沿弧长」、TASK-014 实现约束「必须先 densify 再采样高程」）：
 ...
 *   渲染时线段严丝合缝无缝隙。查询两次换实现简洁（无需全局点缓存），60k 量级 densify 点的总查询耗时
 *   仍在毫秒级（每次查询是闭式墨卡托反算 + 4 角双线性，无迭代）。
 ...
 *
 * 闭合环处理（TASK-014 输出约束「闭环」）：环视为闭合——边 i 的终点 = 边 (i+1)%n 的起点，最后一条边
 * 从 ring[n-1] 回到 ring[0] 闭合。无论源环是否显式首尾重合（契约不强制），都按闭合环处理：显式重合时
 * 接缝边为零长度、被跳过；不重合时接缝边正常 densify 闭合。
 */
 ...
 * 2. 逐省、逐环：投影到世界 (x,z) → 沿世界弧长 densify（间距 = spacing）→ 逐 densify 顶点 queryAtWorld
 *    得真实 h → world_y = h·k + epsilon → 输出子段端点平铺数组。
