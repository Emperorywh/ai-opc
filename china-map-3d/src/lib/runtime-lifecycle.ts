/**
 * 大屏长时运行恢复生命周期的确定性状态机与 resize 防抖（领域层，TASK-022）。
 *
 * 角色与依赖方向：
 ...
 *   次提交且最终尺寸 = 最后一次输入」「监听器清理后事件不再改状态」等不变量（TASK-022 验证方式 1、2、3），
 *   无需启动浏览器 / WebGL（人工恢复与连续 resize 验收留给 TASK-022 验证方式 5）。
 *
 * 生命周期状态迁移（集中编排，TASK-022 实现约束「生命周期状态必须集中编排，场景层和 UI 只消费状态」）：
 ...
 *   不各自监听 context 事件、不各自维护阶段（TASK-022 实现约束「禁止每个组件各自监听 context 或 resize」）。
 *
 * GPU 资源恢复与 CPU 领域数据生命周期分离（TASK-022 实现约束「GPU 资源恢复与 CPU 领域数据生命周期分离，
 *   不得因 context 丢失重复解码 32MB 高程数组或重复下载资产」）：
 * - 本状态机不持有 / 不接触 CPU 高程像素（Uint16Array）或任何资产数据——它只产出阶段。CPU 领域资产由
 *   各资产 hook（useHeightmap 等）在组件树根部一次性加载，跨 context 丢失 / 恢复保持同一引用（React state
 ...
 *   不是「是否需要重新取数」的信号。
 *
 * resize 防抖（TASK-022 实现约束「resize 防抖不得吞掉最终尺寸，不得依赖浏览器自动刷新解决错乱」）：
 ...
 *
 * 无分配 / 无隐式状态（TASK-022 实现约束「挂载 / 卸载、丢失 / 恢复期间不存在重复监听器、重复 Clock 或
 *   运行时几何分配循环」）：
 ...
 *   回退旧实现（TASK-022 实现约束「恢复失败必须显式暴露，不提供低清、平面、旧资产或远程 fallback」）。
 *
...
export function reduceRuntimeLifecycle(
  ...
  switch (event.type) {
    ...
    case 'restore-failed':
      ...
      // 不进入空白死循环（TASK-022 输出约束「恢复失败时显示可诊断状态，不进入空白死循环」）。
 ...
 *
 * 供 RuntimeStatusOverlay 决定是否覆盖可诊断信息（TASK-022 输出约束「恢复失败时显示可诊断状态」）。
 */
 ...
 * 据此决定是否冻结视觉状态推进，不由组件私自监听 context 事件（TASK-022 实现约束「场景层只消费状态」）。
 *
 * 字段刻意**非 readonly**（与 EntranceFrame 同理，TASK-022 实现约束「无运行时分配循环」+ SPEC §7.4）：
 * RuntimeLifecycleController 的单一 useFrame 每帧「原地改写」本对象的 phase / paused 字段，而非整对象替换
 * （60fps × 24h 整对象替换 = 千万级短命对象，GC 抖动）。对象本身由 ChinaMapScreen 的 useRef 一次性创建、
 * 同 fiber 跨重渲染保持同一引用，原地改写零分配。
 */
 ...
 * （TASK-022 实现约束「resize 防抖不得吞掉最终尺寸」）。非法尺寸（非有限 / 非正）被忽略，不污染 pending。
 */
