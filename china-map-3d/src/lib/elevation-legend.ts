/**
 * 海拔色阶图例的数据准备（领域层，TASK-021，SPEC §9 / §3.1）。
 *
 * 角色与依赖方向：
 * - 本模块属于运行时访问层（src/lib），把「图例配置（src/config/elevation-legend 的关键刻度海拔 + 采样段数 +
 *   色阶域）」确定性地变换为「色条 CSS 渐变的 color stop 序列 + 关键刻度的归一化位置 / 颜色 / 文字」，
 *   供渲染层（src/components/ui/ElevationLegend 的 DOM overlay）只消费、不再计算。
 * - 单向依赖：配置层 src/config/elevation-legend（呈现常量）、TASK-010 色阶唯一事实源
 ...
 *   React / R3F / Three.js / DOM / hover 状态——本模块是纯函数，可在 Node 环境完整断言「色条与刻度颜色
 *   来自同一采样器、位置来自同一归一化、六个关键刻度齐全且升序」（TASK-021 验证方式 1）。
 *
 * 色阶复用——单一事实源的形式证明（TASK-021 实现约束「图例不得复制色阶断点和颜色，必须从 TASK-010 的
 *   单一事实源派生」、完成标准「图例与 shader 色阶单一事实源可由自动化证明」）：
 * - 色条 color stop 的颜色 = sampleElevationColor(该 stop 对应的真实海拔)；关键刻度的颜色 =
 *   sampleElevationColor(刻度海拔)。二者都用**同一**采样器——与地表片元着色器（ChinaTerrainMesh 经
 *   resolveElevationColorConfig 派生 256×1 ramp、片元按真实 h 归一化采样）同一事实源。本模块不内置任何
 *   断点 / 颜色字面量，不存在第二套色阶。
 * - 色条 color stop 的纵向位置 = normalizeElevationToRampU(真实海拔, minH, maxH)；关键刻度的位置同一公式。
 *   该归一化与 shader 片元归一化（u = (h−minH)/(maxH−minH)）严格一致，故同一海拔在色条与地表处于同一
 *   相对位置——「色条上 1000m 处的颜色」≡「地表真实 1000m 处的颜色」，可由自动化逐刻度断言。
 * - 色阶域 minH/maxH 引用 elevation-color-ramp 的 ELEVATION_COLOR_DOMAIN（与 shader 经
 *   resolveElevationColorConfig 复核 meta 上下限所对照的同一冻结常量），不复制数值。
 *
 * 输出形态（渲染层 DOM overlay 直接消费的稳定产物）：
 * - barStops：色条 CSS linear-gradient 的 color stop 序列，按位置升序、首末分别落在色阶域下限 / 上限。
 *   渲染层据此拼 `linear-gradient(to top, stop0, stop1, ...)`（to top = 低海拔在底、高海拔在顶）。
 * - ticks：关键刻度数组，每项含海拔、归一化位置（0=色条底、1=色条顶）、该海拔颜色（十六进制，供刻度
 *   色块 / 描边复用）、文字标签。渲染层据此在色条侧边标注刻度。
 */
 ...
 *
 * positionFraction ∈ [0,1]：0 = 色阶域下限（深海近黑，色条底部）、1 = 色阶域上限（雪白，色条顶部）。
 * 由 normalizeElevationToRampU(真实海拔, minH, maxH) 派生——与 shader 片元归一化同一公式。
 * colorHex = sampleElevationColor(真实海拔) 转 #rrggbb——与地表片元着色器同一采样器。
 */
 ...
 * 准备好的单个关键刻度（读图辅助）。
 *
 * 颜色与位置都不硬编码：颜色 = sampleElevationColor(海拔)，位置 = normalizeElevationToRampU(海拔, minH, maxH)，
 * 二者与地表着色器同源。label 为刻度文字（如「1000」），渲染层在后面拼接单位。
 */
...
export interface PreparedElevationLegend {
  ...
  /** 关键刻度数组（按海拔升序），颜色 / 位置均从 TASK-010 色阶唯一事实源派生。 */
...
/**
 * 把海拔色阶图例配置确定性地准备为渲染层可消费的色条 + 关键刻度（颜色 / 位置全部从 TASK-010 色阶唯一
 * 事实源派生）。
 *
 * 流水线：
 ...
 *
 * 色阶域来自 ELEVATION_LEGEND_CONFIG.domain（即 elevation-color-ramp 的 ELEVATION_COLOR_DOMAIN 冻结常量），
 * 与 shader 经 resolveElevationColorConfig 复核 meta 上下限所对照的域同一事实源——本函数不接收 minH/maxH
 * 入参，避免引入第二套域。所有颜色由 sampleElevationColor 现场取得，本函数不内置断点 / 颜色字面量。
 */
export function prepareElevationLegend(): PreparedElevationLegend {
  ...
  // 色条 color stop：均匀采样「段数 + 1」个海拔。i=0 → minH（深海近黑，色条底）、i=段数 → maxH（雪白，顶）。
  // 颜色与位置都从 TASK-010 唯一事实源派生，不存在第二套色阶。
