/**
 * SPEC §6 政治边界红线点名缺项的共享扫描（领域层，TASK-019 抽取）。
 *
 * 角色与依赖方向：
 * - 本模块属于运行时访问层（src/lib），只依赖契约层 src/geo-contracts（PoliticalBoundaryContract
 *   类型）与同层红线点名领域真值 political-catalog（REQUIRED_NINE_DASH_SEGMENT_INDICES /
 *   TAIWAN_EAST_SEGMENT_INDEX / REQUIRED_ISLAND_NAMES）。不依赖 React / R3F / Three.js / DOM /
 *   配置层 / 其他领域计算——是一份「只读消费红线目录、产出缺项事实」的纯函数。
 *
 * 为什么单独成模块（避免两套红线扫描逻辑）：
 * - 主图十段线 / 岛礁准备（src/lib/political-features.assertRedLineCompleteness）与 2D 南海附图准备
 *   （src/lib/south-china-sea-inset.prepareSouthChinaSeaInset）都要对同一份政治边界契约做
 *   「SPEC §6 红线点名项是否齐全」的判定。若两处各自遍历 features、各自对照目录常量，就会出现两份
 *   等价的扫描代码——一旦其中一处漏判（如只查段数、漏查台湾东侧段独立锚点），残缺地图会在那一处
 *   静默生成，违反 SPEC §6 红线。
 * - 故把扫描逻辑抽成一份纯函数 collectPoliticalRedLineGaps：遍历契约 features，对照红线目录常量，
 *   返回结构化缺项（段数 / 缺段序号 / 台湾东侧段是否在 / 缺点名岛礁名）。两处消费者各自据缺项抛自己
 *   的稳定错误码（political-features.* / south-china-sea-inset.*），因此：
 *     · 红线「目录」（点名了哪些段 / 岛礁）唯一来自 political-catalog；
 ...
 *   一个只读消费者 + 一份共享扫描实现。
 *
 * 扫描语义（与原 political-features.assertRedLineCompleteness 逐项等价，行为不变）：
 ...
 *
 * 调用方（主图准备 / 附图准备）据返回的缺项各自抛稳定错误码，故本函数只负责「如实算出缺什么」。
 * 扫描逻辑单一来源于此：消费者不再各自遍历 features、各自对照目录常量（详见模块头注释）。
 *
