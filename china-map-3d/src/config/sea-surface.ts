 ...
 *   这些参数——禁止在组件 / 着色器 / 测试里各自复制一份海平面高度或透明度（TASK-013 实现约束
 *   「海面作为独立渲染层」「不得隐式状态、跨层耦合」）。
 ...
 *   「透明度落在 [0.55, 0.7]」「覆盖范围 = 主图世界包围盒」「动画参数有限」等不变量
 *   （TASK-013 验证方式 1）。
 *
 * 海平面 = 地形海平面（同一米制 y=0，TASK-013 实现约束「海面必须与地形高程使用同一米制 y=0 海平面，
 * 不得用视觉偏移掩盖坐标不一致」）：
 * - 地形 vertex shader 把真实海拔 h 位移到世界 y = h·k（SPEC §3.2）；h=0（海平面）时世界 y=0。
 *   故海面 plane 放在世界 y=0 即恰好落在地形的海平面——二者共用同一米制海平面，无需任何视觉偏移。
 * - SEA_LEVEL_Y_METERS = 0 是这一不变量的显式锚点：组件据此放置 mesh（position.y=0），测试据此断言
 *   「海平面 y = 0」。任何把海面抬 / 降以掩盖坐标不一致的改动都会改本常量，从而被测试捕获。
 *
 * 半透明基线（SPEC §3.5「opacity ≈ 0.55–0.7」、TASK-013 输出约束「透明度基线位于 0.55–0.7」）：
 * - 取 0.6（区间内）：既能透过海面看到水下大陆架（保留负高程地形可见），又足够「是水」而非虚无。
 *   透明度是基线 uniform（uOpacity），片元着色器直接作为输出 alpha——不参与陆地色阶、不随高程变化。
 * - 半透明 + depthWrite=false（见组件）：海面在透明通道绘制、不写深度，使水下地形（不透明、已写深度）
 *   透过海面可见，且海面不会遮挡陆地（陆地 world-y>0 在海面之上，深度测试使海面片元在陆地区域被丢弃）。
 *
 * 大陆架透视由「水下地形色阶 + 海面半透明」共同呈现，不由海面着色（TASK-013 实现约束「不得让海面
 * 参与陆地色阶」、验证方式 2「负高程仍由地形层保留，海面层不会改写高程或色阶配置」）：
 * - 水下地形（h<0）由地形层按高程色阶着成深海近黑（近岸 h 接近 0 偏亮、远海 h 接近 -1500 近黑，
 *   SPEC §3.1 / §3.5）。海面只是覆盖其上的半透明面——透视看到的「近岸浅、远海深」梯度来自水下
 *   地形本身，海面着色恒为深蓝青基线色 + 细微扰动，**不**读取 heightmap、**不**重做色阶。
 * - 故本模块**不**携带任何高程 / 色阶字段（domain / ramp / breakpoints / minH / maxH），测试断言其上
 *   不存在这些字段，证明海面层不会改写高程或色阶配置（TASK-013 验证方式 2）。
 *
 * 双层流动扰动（SPEC §3.5「双层流动 noise / gerstner 概念简化」、TASK-013 输出约束「统一渲染时钟驱动
 * 双层流动扰动或等价低成本微波效果」）：
 * - 两层正弦波（不同 UV 频率、不同方向、不同流速、不同相位）叠加，对海面基线色做细微亮度调制，
 *   模拟微波涟漪。幅度刻意压低（< 0.1），保证「细微、不喧宾夺主」（TASK-013 实现约束）。
 * - 两层都消费**同一个** uTime uniform（由 R3F 共享时钟驱动，见 SeaSurface 组件）——这是「统一时钟、
 *   不建独立漂移时钟」的结构性保证：着色器只声明一个 uniform float uTime，两层波动的时间项都是
 *   uTime·speed。测试据此断言「着色器内 uTime 声明恰一次」「两层 sin 均含 uTime 项」。
 *
 * 海面 plane 覆盖范围 = 主图世界包围盒（与地形 plane 同一份世界范围）：
 * - 海面必须覆盖目标海域且与地形 plane 共面同范围（TASK-013 输出约束「覆盖目标海域且不改变水下地形
 *   网格」）。SEA_SURFACE_PLANE_LAYOUT 由 MAIN_MAP_WORLD_BOUNDS 派生（与 src/three/terrain-layout
 *   的 TERRAIN_PLANE_LAYOUT 同一公式），测试断言二者相等——保证海面与地形逐米对齐，无第二套范围常量。
 */
 ...
 * （TASK-013 实现约束「不得用视觉偏移掩盖坐标不一致」）。单独导出此常量使「海平面 = 0」成为
 * 可读、可测的不变量锚点（测试断言其为 0）。
 */
 ...
 * 基线（uColor uniform），不读 heightmap、不参与陆地色阶——大陆架透视看到的深浅梯度来自水下地形色阶
 * 透过半透明海面，而非海面自身着色。
 */
 ...
 * 第一层流动波（低频长波，主导微波方向感）。
 *
 * 频率 / 流速 / 幅度均为内部调参（TASK-013 实现约束「具体波形实现属于内部选择」），仅受「幅度 < 0.1
 * （细微）」「参数有限」「两层消费同一 uTime」三道不变量约束（测试断言）。
 */
 ...
 * - widthX：plane 在世界 x（东）方向的米制跨度 = maxX − minX。
 * - heightZ：plane 在世界 z（南）方向的米制跨度 = maxZ − minZ。
 * - centerZ：mesh 在世界 z 方向的定位，使 plane 覆盖 [minZ, maxZ]；x 关于原点对称故 mesh x=0。
 *
 * 海面与地形必须共面同范围（TASK-013 输出约束「覆盖目标海域且不改变水下地形网格」）：测试断言
 * SEA_SURFACE_PLANE_LAYOUT 与 TERRAIN_PLANE_LAYOUT 逐字段相等——保证海面与地形逐米对齐，不存在
 * 第二套范围常量导致的海陆错位。
 */
export const SEA_SURFACE_PLANE_LAYOUT = Object.freeze({
  widthX: MAIN_MAP_WORLD_BOUNDS.maxX - MAIN_MAP_WORLD_BOUNDS.minX,
  heightZ: MAIN_MAP_WORLD_BOUNDS.maxZ - MAIN_MAP_WORLD_BOUNDS.minZ,
 ...
 * 海面 plane 每边分段数。
 *
 * 波动是片元着色器内的亮度调制（SPEC §3.5「fragment shader 用时间驱动的法线扰动」），不需要顶点位移，
 * 故 plane 只需 1 段（2 三角形）即可覆盖整个海域——顶点预算极低，与地形 2048² 分段无关。海面是独立
 * 渲染层，分段不与地形共享 / 不影响地形网格。
 */
 ...
 * 海面渲染的全部参数（冻结）。
 *
 * 这是海面装配（SeaSurface）、海面着色器 uniform 与自动化测试共享的同一份事实源：海平面 y / 透明度·
 * 上下限 / 基线色·字节 RGB / plane 覆盖范围 / 分段 / 双层波动参数全部在此，不存在第二套海面常量。
 * 冻结防止运行时被偷偷改（如把透明度调到 1.0 会变不透明遮住大陆架），任何调整都必须改本模块并同步测试。
 */
