...
本文档说明 china-map-3d 的**共享 CPU 高程查询领域层**：如何把 TASK-003 交付的 16 位 heightmap
资产在运行时只解码一次，向省界贴地、标签定位、遮挡判断（SPEC §7.5）、海陆判断（SPEC §3.5）等
所有领域消费者提供「真实米制海拔 + 双线性采样」的统一查询能力。SPEC §3.6 的「CPU 端 heightmap」
与 §7.5 的「遮挡射线」明确要求共用同一份高程事实源；本模块即该事实源的运行时入口。
...
- GPU 顶点位移纹理可以是「另一份表示」（GPU 持有 R16 纹理，片元 / 顶点着色器自行采样），但 CPU 与
  GPU **解码语义必须一致**：二者都用 `decodeUint16ToElevation(code, min, max)` 把 uint16 编码还原成
  真实米制，且双线性对「归一化编码」的仿射性保证 CPU 先解码四角再插值 == GPU 先硬件双线性再解码
  （见 §3）。本模块是 CPU 侧唯一查询入口，不存在第二套 CPU 解码。
...
UV 约定（与 `src/lib/projection` 末段「heightmap 行 0=北、列 0=西；u 随 +X/东增，纹理 v 对应北→南
行序」、与 `docs/projection.md` §6 严格一致）：
...
GPU 在归一化 R16 纹理上做硬件双线性、再在着色器里线性解码到米的结果在浮点精度内一致。因此 CPU 与
GPU 共用同一高程事实源且语义相同（TASK-008 输出约束）。
...
**结果类型**：查询返回 `ElevationQueryResult` 判别联合——成功带 `meters`（真实海拔，米）与按符号划分的
`kind`（`below-sea-level` / `sea-level` / `above-sea-level`），失败带稳定 `code` 与简体中文 `message`。
**失败绝不伪装成 `meters:0` 的成功**——海平面 0m 是合法读数，异常 / 越界 / 已释放必须显式区分
（TASK-008 输出约束「不以魔法 0 混淆异常与海平面」）。

**加载期错误**：`ElevationProviderError`（带稳定 code：`elevation.meta-invalid` /
`elevation.raster-size-mismatch` / `elevation.decode-byte-length-mismatch`），调用方在取得 provider 前
即可确定性地发现坏元数据 / 坏栅格；加载失败时**绝不**静默落到平地 fallback（SPEC 红线）。
...
- `queryAtWorld`：先 `invertWorld`（TASK-007）反算经纬度，再走 `queryAtLonLat`。反投影失败透传为
...
回退本 TASK 只会移除 `src/lib/elevation.ts`、`tests/elevation.test.ts` 与本文档；统一投影
（TASK-007）与所有静态资产（边界、地点、政治边界、heightmap `.r16` + 元数据）仍可独立验证，不会
受到破坏——资产不依赖本模块的存在，GPU 顶点位移也可独立从 `.r16` 重新解码。
