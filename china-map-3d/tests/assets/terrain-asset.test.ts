 ...
 * 证明 16 位精度与真实米制解码未丢失、关键地势相对关系成立、篡改元数据会被资产校验发现。
 *
 * 注意：篡改类用例一律在内存副本上构造，绝不改写 public/ 下的正式资产（TASK-003 验证方式 4：
 * 「不得改动正式资产验证异常路径」）。
 */
...
/** 生产资产载荷：元数据 + 栅格像素 + 原始字节 + 来源注册表 + 审计 sidecar。模块级缓存，避免每个用例重读 33MB。 */
...
function loadProductionAsset(): ProductionAsset {
  ...
  const sources = JSON.parse(readFileSync(resolve(projectRoot, SOURCES_PATH), 'utf-8'))
...
describe('16 位编码端到端解码（TASK-003 验证方式 2）', () => {
  /**
...
describe('生产高程资产地势抽样不变量（TASK-003 验证方式 3）', () => {
  ...
  it('16 位精度保持：不同 uint16 编码数远超 8 位上限 256，未被静默降为 8 位', () => {
    expect(outcome_samples_distinctCodes()).toBeGreaterThan(1000)
  })

  function outcome_samples_distinctCodes(): number {
    return verifyTerrainAsset({
      meta: asset.meta,
      pixels: asset.pixels,
      width: asset.width,
      height: asset.height,
    }).samples.distinctCodes
  }
})

describe('篡改元数据后资产校验确定性失败（TASK-003 验证方式 4）', () => {
  ...
  /**
   * 精度丢失检测（TASK-003 实现约束：校验必须能发现被静默降为 8 位的栅格）。
