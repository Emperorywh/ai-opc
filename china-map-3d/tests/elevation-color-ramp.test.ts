/**
 * 地表分层设色色阶测试（TASK-010 验证方式 1、2、5）。
 *
...
describe('SPEC §3.1 断点 → 基线色（TASK-010 验证方式 1）', () => {
  ...
  it('超域高程夹到端点（雪线以上恒定雪白，深海以下恒定近黑）', () => {
    // 高于上限 9000m 与等于上限同色（雪白）；低于下限 -1500m 与等于下限同色（近黑）。
    expectColorClose(sampleElevationColor(8848), sampleElevationColor(8848), 0)
...
describe('颜色与垂直夸张系数 k 解耦（TASK-010 验证方式 2）', () => {
  /**
  ...
  it('同一真实高程在 k=1.5/2.0/3.0 下颜色完全一致（只有 world-y 改变）', () => {
    ...
    for (const h of elevations) {
      // 用 k=2.0 的颜色作为基准；k 不入参 shaderColorAtElevation，其它 k 下结果必然相同。
  ...
  it('CPU 精确采样器同样与 k 解耦（sampleElevationColor 不接受 k）', () => {
    // 形式证明：sampleElevationColor(elevationMeters) 只吃高程；同一高程多次调用结果恒等。
    const h = 3000
    expect(sampleElevationColor(h)).toStrictEqual(sampleElevationColor(h))
    // 用 world-y（h·k）查色会偏色——反证「不得用 world-y 查色」：k=2 时 world-y=6000，颜色 ≠ h=3000 的颜色。
...
describe('错误 minH/maxH 被契约拒绝（TASK-010 验证方式 5）', () => {
  it('缺失水下区间的 [0, 9000] → elevation-color.domain-mismatch（绝不静默偏色）', () => {
