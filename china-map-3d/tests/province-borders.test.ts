/**
 * 省级贴地边界准备层测试（TASK-014 验证方式 1、2、3）。
 *
 * 依赖方向：测试基线（vitest，Node 环境），import src/lib/province-borders（领域准备层）、src/lib/elevation
 * （createElevationProvider 构造合成 provider）、src/lib/projection（projectToWorld / invertWorld 复算与校验）、
 * src/geo-contracts（encodeElevationToUint16 编码 + 几何类型）。不依赖浏览器 / React / Three.js——准备层是
 * 纯函数，可在 Node 内完整断言 densify 间距、贴地 y = h·k+epsilon 不变量、多多边形 / 闭环 / 岛屿保留、
 * 负高程、与各类失败路径（无效输入 / 投影失败 / 查询失败 / 退化），无需启动 WebGL
 * （人工视觉验收留给 TASK-014 验证方式 4、5）。
 *
 * 覆盖（TASK-014 验证方式 1、2、3）：
 * - densify 间距：长边按 ceil(边长/间距) 细分，每个子段长度 ≤ 间距；短边整段 1 子段。
 * - 贴地不变量：每个 densified 端点 y === provider.queryAtWorld(x,z).meters · k + epsilon（精确相等——
 *   prep 与测试走同一 queryAtWorld 路径、同一算术）。
 ...
 * - 集成：真实 34 省资产 + 合成 provider 跑通，产出 34 个行政区边界、每个非零，无抛错。
 */
...
describe('densify 间距：长边按 ceil(边长/间距) 细分，子段长度 ≤ 间距（TASK-014 验证方式 1）', () => {
...
describe('贴地不变量：y = queryAtWorld(x,z).meters · k + epsilon（精确相等）', () => {
...
describe('多多边形 / 岛屿 / 内环：所有环边界完整保留（TASK-014 验证方式 2）', () => {
...
describe('闭环：环视为闭合，接缝段回到首顶点（TASK-014 输出约束「闭环」）', () => {
...
describe('失败路径：不产出平地边界（TASK-014 验证方式 2 / 输出约束）', () => {
...
describe('集成：真实 34 省资产 + 合成 provider 跑通（TASK-014 验证方式 1）', () => {
  it('真实 china-provinces-geometry.json 经契约校验 + densify + 贴地产出 34 个行政区边界', () => {
    ...
    const contract = payload as { features: readonly AdministrativeGeometryFeature[] }
    // 用常数高程 provider（真实范围）+ 生产间距跑全量 densify + 贴地。
    const provider = makeConstantProvider(1000)
    const result = prepareProvinceBorders(contract.features, provider, 2, PREP_CONFIG)
    // 恰好 34 个行政区边界。
  ...
  it('真实 34 省边界端点 y 全部等于 meters·k + epsilon（贴地不变量在全量数据上成立）', () => {
    ...
    const k = 2
    const epsilon = PREP_CONFIG.terrainEpsilonMeters
    const result = prepareProvinceBorders(payload.features, provider, k, PREP_CONFIG)
    ...
    expect(sample.ok).toBe(true)
    const expectedY = sample.ok ? sample.meters * k + epsilon : NaN
    // 抽样若干端点断言（全量遍历 34 省所有端点亦可，但抽样足以证明不变量）。
    let checked = 0
    for (const border of result.borders) {
...
describe('配置不变量：抗 z-fighting / 海面共存的结构性前提（TASK-014 验证方式 1、5）', () => {
