/**
 * 标签地形遮挡判定测试（TASK-017 验证方式 1、2）。
 *
 * 依赖方向：测试基线（vitest，Node 环境），import src/lib/label-occlusion（领域纯函数 computeLabelVisibility
 * + TerrainWorldYSampler 闭包类型）、src/config/label-occlusion（生产配置不变量）、src/lib/elevation
 * （createElevationProvider 构造真实 provider，验证「provider→采样器」适配与 released → indeterminate）、
 * src/geo-contracts（encodeElevationToUint16 + TerrainMetaContract 构造合成 heightmap）。不依赖浏览器 /
 * React / Three.js / troika——判定层是纯函数，可在 Node 内用确定性几何夹具完整覆盖「无遮挡 / 前方山体
 * 遮挡 / 命中点位于标签之后 / 射线擦边 / 相机移动 / 生命周期与无分配」等场景，无需启动 WebGL
 * （人工视觉验收留给 TASK-017 验证方式 4、5）。
 *
 * 覆盖（TASK-017 验证方式 1、2）：
 * - 无遮挡：视线全程高于地形 → visible。
 * - 前方山体遮挡：标签与相机之间存在高过视线的地形峰 → occluded（命中点比标签更近相机）。
 * - 命中点位于标签之后：地形峰在标签之后（远离相机一侧）→ 不被采样 → visible。
 * - 射线擦边：地形峰刚好抵近视线但未超过 verticalClearance → visible（抗擦边抖动）；越过余量 → occluded。
 * - 相机移动：同一地形下，相机姿态 A 遮挡、姿态 B 可见（状态随相机确定转换）。
 * - 生命周期 / 无分配 / 无共享状态：退化射线 / 全失败采样 / 非有限输入 → indeterminate；released provider
 *   → 采样器返回 null → indeterminate；重复调用确定性一致；交错调用无交叉污染；高频调用不抛错。
...
describe('无遮挡：视线全程高于地形 → visible（TASK-017 验证方式 1）', () => {
...
describe('前方山体遮挡：标签与相机之间存在高过视线的地形峰 → occluded（TASK-017 验证方式 1）', () => {
  it('水平视线被中段山体挡住（命中点比标签更近相机）→ occluded', () => {
    // 标签 (0, 100, 0)，相机 (0, 100, 1000)：射线水平于 y=100。山体在 z∈(400,600) 高 200，高过视线。
  ...
  it('略微下倾的视线被中段山体挡住 → occluded', () => {
    // 标签 (0, 120, 0)，相机 (0, 80, 1000)：射线 y 从 120 降到 80。中段 z≈500 处射线 y≈100，山体 200 仍高过。
    // 山体宽 240（380–620），保证 8 样本离散化下至少一个样本落入山体范围。
    const vis = computeLabelVisibility(
...
describe('命中点位于标签之后：山体在标签之后（远离相机）→ 不被采样 → visible（TASK-017 验证方式 1）', () => {
  it('山体峰在标签背后（z<0，远离相机）→ 射线区间不覆盖 → visible', () => {
    // 标签 (0, 100, 0)，相机 (0, 100, 1000)：射线采样 z∈(10,990)。山体在 z∈(-300,-100)（标签之后）。
...
describe('射线擦边：地形刚好抵近视线 → verticalClearance 决定结论（TASK-017 验证方式 1）', () => {
  ...
  it('地形高过视线刚好等于余量边界（103 vs clearance 5 → 实际需 >105）→ visible', () => {
    // 山体 105：判定条件 terrainY > py + clearance = 100 + 5 = 105，105 不大于 105 → visible（严格大于）。
    const vis = computeLabelVisibility(
...
describe('相机移动：同一地形下状态随相机确定转换（TASK-017 验证方式 1）', () => {
...
describe('生命周期 / 退化路径：不产生错误射线（TASK-017 验证方式 2）', () => {
  ...
  it('采样器部分失败：失败点跳过，但存在遮挡点 → occluded', () => {
    // z<400 返回 null；z∈[400,600] 返回 200（遮挡水平视线 100）；z>600 返回 10。
...
describe('无分配 / 无共享状态：可被帧循环高频调用（TASK-017 验证方式 2）', () => {
...
describe('集成：真实 ElevationProvider → 采样器适配（TASK-017 验证方式 2）', () => {
  ...
  /**
   * 场景层「provider + k → 采样器」适配的同一构造（见 ChinaMapScreen.PlaceLabelsLayer）。
  ...
  it('真实 provider 在主图世界包围盒内查询成功，采样器返回 h·k', () => {
    ...
    // 取主图世界包围盒中心附近一点（地图原点附近，必在 heightmap 范围内）。
  ...
  it('真实 provider 驱动判定：标签高于地形、相机更高 → visible', () => {
    ...
    const sampler = makeSampler(provider, 2)
    // 标签在原点上方（h·k≈1000）再加浮高 5000；相机更高 20000；地形恒 h·k≈1000。视线远高于地形。
    const vis = computeLabelVisibility(
...
describe('配置不变量（TASK-017 验证方式 1）', () => {
