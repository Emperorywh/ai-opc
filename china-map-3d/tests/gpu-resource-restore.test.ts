/**
 * GPU 资源恢复遍历测试（TASK-022 验证方式 3「重复 context 丢失 / 恢复，预期 GPU 资源所有权数量稳定」）。
 *
...
describe('restoreMaterialGpuResources：CPU 源数据生命周期分离（不重新解码 / 下载）', () => {
  it('DataTexture 的 .image.data 引用遍历前后不变（复用同一份 CPU 像素，不重新分配）', () => {
    // 模拟 TASK-008 的 32MB CPU 高程像素：一份 Uint8Array（测试用小尺寸）。
    ...
    // 关键不变量（TASK-022 实现约束）：GPU 资源恢复只对纹理置 needsUpdate（version++），CPU 源数据
    // （.image.data）引用不变——绝不重新 fetch / 重新解码 .r16。遍历函数不接触 / 不替换 .image.data。
