/**
 * 地形渲染配置与 CPU/GPU 解码一致性测试（TASK-009 验证方式 1、5）。
 *
 ...
 * - 位移语义：y = h·k；改 k 只改 y，不改真实高程 h（TASK-009 输出约束）。
...
describe('生产默认未被偷偷改低（TASK-009 输出约束）', () => {
...
describe('CPU/GPU 解码一致性 + 16 位精度（TASK-009 输出约束）', () => {
  /**
  ...
  it('decodeNormalizedToElevation(code/65535) == decodeUint16ToElevation(code)，全位深样本', () => {
    ...
    for (const code of codes) {
      ...
      expect(
        elevationWithinTolerance(byConfig, byContract, 1e-9),
...
describe('位移语义：y = h·k，改 k 只改 y（TASK-009 输出约束）', () => {
  ...
  it('同一真实高程 h，k=1.5/2.0/3.0 只改变 y，不改 h（验证步骤 4 不变量）', () => {
