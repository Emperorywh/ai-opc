 ...
 * 契约层与同目录的 province-catalog（34 省领域真值）。严禁依赖浏览器 / React / Three.js
 * 或任何运行时状态。本脚本只在「资产生产期」联网取数，产物落盘后运行时零外部网络依赖
 * （TASK-004 实现约束：不得在运行时访问 DataV）。
 *
 * 几何修复边界（明确范围，避免越权）：
 * - 只做「格式转换 + 非省级要素过滤」：把 GeoJSON 的 [lon,lat] 数组改为契约命名字段
 *   {lon,lat}；把 adcode 命中 34 省目录的要素纳入资产，**忽略** 100000_JD（九段线）等
 *   非省级要素——九段线 / 南海岛礁 / 钓鱼岛 / 藏南 / 阿克赛钦等国标完整性是 TASK-006 的红线。
 * - **不**做拓扑修复（自相交、缝隙、重叠）、**不**做简化或抽稀、**不**补充任何政治要素。
 *   DataV 已知缺陷（部分争议区画法非国标、不含九段线 / 南海岛礁）原样保留，由 TASK-006 闭环。
...
interface DataVFeature {
  geometry: { type: 'Polygon' | 'MultiPolygon'; coordinates: GeoJsonPolygon[] | GeoJsonPolygon[][] } | null
...
function convertGeometry(feature: DataVFeature): AdministrativeGeometry {
  ...
  if (geom.type === 'Polygon') {
    const rings = (geom.coordinates as GeoJsonPosition[][]).map(convertRing)
    return { type: 'Polygon', rings }
  }
  // MultiPolygon：每个多边形独立拥有自己的环列表。
  const polygons = (geom.coordinates as GeoJsonPosition[][][]).map((polygon) => ({
...
async function main(): Promise<void> {
  ...
  const provenance = {
    ...
    generation: {
      ...
      catalog: 'scripts/provinces/province-catalog.ts（34 省领域真值）',
      processingParams: {
        geometryConversion: 'GeoJSON [lon,lat] → 契约 {lon,lat} 命名字段，类型保留（Polygon / MultiPolygon）',
        topologyRepair: 'none（不做拓扑修复 / 抽稀 / 政治要素补充；九段线 / 南海岛礁 / 争议区由 TASK-006 独立闭环）',
