/**
 * 省级边界资产级深度校验（TASK-004）。
 *
 * 依赖方向：属于离线资产生产 / 校验层（scripts/verify-assets，tsx 运行），单向依赖
 * src/geo-contracts 契约层与同层 scripts/provinces/province-catalog（34 省领域真值）。
 ...
 * 调用同一函数。
 *
 * 与 TASK-001 契约校验的关系：契约校验（validateAdministrativeDirectory / validateAdministrativeGeometry）
 * 只验目录与几何的字段结构；本模块在其之上追加「资产级」不变量——
 *   恰好 34 省、港 / 澳 / 台必在、目录与几何一一对应（双射）、与 34 省目录真值精确一致、
 *   所有环闭合、坐标落在中国主图范围、来源可解析、provenance 完整性摘要逐项一致。
 * 这些是 TASK-004 验证方式 1、3 的落点。
 *
 * 政治红线独立锚点（SPEC §6、TASK-004 验证方式 1「港、澳、台均存在」）：
 ...
 * 校验随之确定性失败。九段线 / 南海岛礁 / 钓鱼岛 / 藏南 / 阿克赛钦完整国标画法由 TASK-006 闭环，
 * 本 TASK 不得越权声称已完成。
 */
...
export function verifyProvincesAsset(input: ProvincesAssetVerificationInput): ProvincesAssetOutcome {
  ...
  // 1) 目录与几何各自的契约结构校验（复用 TASK-001 校验器）。保留原始错误码，
  ...
  // 10) 审计 sidecar 完整性比对（防篡改锚点，与 TASK-003 terrain provenance 同构）。
