 ...
 * 与同目录的 build-heightmap（TASK-002 可测试核心）/ mercator 投影原语。
 * 严禁依赖浏览器、React、Three.js 或任何运行时状态。
 *
 * 为什么用 ETOPO1 而非 SPEC 字面提到的 Copernicus DEM GLO-30（可逆决策，已记录于来源声明与 README）：
 * - 本 TASK 的资产级校验要求「海域包含负高程 / 浅水负高程保留」（TASK-003 验证方式 3、SPEC §3.5 / §5.1）。
 * - Copernicus DEM GLO-30 是数字表面模型，开阔海域为 0 或无效值，**不含海洋水深**，
 *   无法提供校验所需的负高程海域样本。
 * - NOAA ETOPO1 是公开的全球地形 + 水深一体化栅格（1 弧分），同时具备真实陆地高程与海洋水深，
 *   可一次性满足「青藏高原高于东部平原 / 盆地低于周边山地 / 海域含负高程」三类地势抽样不变量。
 * - 取数通过公开的 Open Topo Data etopo1 端点（按经纬度点查询），无需凭据；ETOOPO1 原始栅格托管于
 *   NOAA NCEI（公共领域数据）。来源、版本与免责声明在 public/geo/data-sources.json 与 .provenance.json
 *   中完整记录，资产来源可审计。
 * - 该决策可逆：TASK-002 的 build-heightmap 流水线对输入 DEM 源中立，将来若接入含正式水深的
 *   Copernicus 衍生品或更换更高分辨率源，重跑本脚本即可产出同一外部契约的资产。
 *
 * 可重复性（TASK-003 / TASK-002 衔接）：
 * - 本脚本负责「源数据获取」：按目标范围在规则经纬度网格上抽样 ETOPO1，组装成 dem-tile-fixture-v1
 *   （TASK-002 流水线的既定输入格式），再调用 buildHeightmap 完成 EPSG:4326→EPSG:3857 重投影、
 *   双线性重采样到 4096²、16 位线性编码与元数据导出。
...
/** 把规则网格组装成 dem-tile-fixture-v1（TASK-002 流水线的既定输入格式）。 */
