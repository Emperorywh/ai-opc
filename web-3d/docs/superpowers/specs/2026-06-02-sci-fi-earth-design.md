...
| 图形 API | WebGPU 优先 + WebGL 2 回退 | 未来导向，Three.js 已支持 WebGPU renderer |
| 后处理 | 中等电影感（Bloom + Vignette + Noise） | Bloom 明显但不吃细节 |
| 扫描线 | 不做 | 与纯 3D 沉浸方向矛盾 |
| 星空 | 程序化 Shader（带闪烁和流动） | 性能好，更像真实宇宙 |
| 架构方案 | 自定义 Shader + R3F 骨架 | 视觉天花板高 + WebGPU 友好 + 结构清晰 |
...
Mouse Events ──────────────────────────────────→ useInputPriority
                                                 ...
                                                 CameraController
                                                       ↓
                                                 Camera State (Zustand)
                                                       ↓
                                          Earth rotation + Camera orbit
```
...
| 层 | 来源 | 分辨率 | 用途 |
|---|---|---|---|
| Day map | NASA Blue Marble | 8192×4096（或 4096×2048） | 白天地表 |
| Night map | NASA Black Marble | 8192×4096（或 4096×2048） | 夜晚城市灯光 |
...
| Day map 纹理 | ~15-25MB | 按需加载，渐进式 |
| Night map 纹理 | ~15-25MB | 按需加载，渐进式 |
| MediaPipe Hands WASM | ~5MB | 异步延迟加载（优先级低于纹理） |

总加载量约 35-55MB。首次加载后浏览器缓存可大幅减少后续访问时间。
...
- 所有自定义 Shader 同时支持 GLSL 300 es（WebGL 2）和 WGSL（WebGPU）
- Three.js 的 WebGPU renderer 提供 GLSL → WGSL 自动转换（TSL）
- 复杂 Shader 使用 Three.js ShaderLanguage (TSL) 编写，确保双后端兼容

### 12.3 回退降级

如果 WebGPU 不可用，回退到 WebGL 2。所有核心效果在 WebGL 2 下均可运行，无功能缺失。唯一的差异是 WebGPU 下可能有更好的性能（compute shader 粒子更新等）。
...
interface SceneState {
  ...
  cameraTheta: number;
  cameraPhi: number;
  cameraVelocity: { theta: number; phi: number; zoom: number };
  ...
  // 加载
  loadingPhase: 'particles' | 'texture' | 'activate' | 'done';
  ...
  // Actions
  setInputMode: (mode: 'mouse' | 'gesture' | 'idle') => void;
  recordInput: () => void;
  updateCamera: (delta: Partial<CameraState>) => void;
