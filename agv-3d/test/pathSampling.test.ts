...
/** 在参数 t 处计算三次贝塞尔曲线上的点（独立实现，用于校验采样点是否在曲线上）。 */
...
/** 密集采样真实曲线，用于近似校验扁带折线对曲线的逼近误差。 */
function denseCurve(p0: Point2, p1: Point2, p2: Point2, p3: Point2, n = 4000): Point2[] {
  const out: Point2[] = []
  for (let i = 0; i <= n; i += 1) {
...
/** 点到折线的最近距离。 */
function distToPolyline(p: Point2, polyline: readonly Point2[]): number {
