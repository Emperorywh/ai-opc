 ...
 *   ACES 色调映射与曝光 1.0，原生 antialias 关闭；PostEffects 接入唯一 Bloom → SMAA → ToneMapping 链，
 *   Composer multisampling 为 0、Bloom 阈值 1.0 仅触发流动高亮与高发光节点，基础路径与背景不 Bloom。
 *   EffectComposer 渲染期把 renderer.toneMapping 置 NoToneMapping，ACES 由链路末端 ToneMappingEffect
 *   补回一次；着色器与后处理之间不重复 tone mapping 或色彩空间转换（详见 PostEffects）。
 *
...
export function MapSceneView({ packet, scene, initiallyReady }: MapSceneViewProps) {
  ...
  // 挂载期会把 renderer.toneMapping 置 NoToneMapping（渲染期恒等，供 Bloom 阈值在线性 HDR 上触发，
  // 卸载时恢复此处的 ACES）；渲染期 ACES 改由链路末端 ToneMappingEffect 补回一次（见 PostEffects）。
  ...
  return (
    <div className="agv-map-scene" style={{ opacity }}>
      ...
      >
        ...
        {/*
          SPEC §8.1 PostEffects、§8.5（TASK-014）：唯一后处理链 Bloom → SMAA → ToneMapping，接管渲染循环。
          放在 CameraRig 之后、作为场景级管线（非数据驱动图层）；EffectComposer 渲染期把 renderer.toneMapping
          置 NoToneMapping，ACES 由链路末端 ToneMappingEffect 补回一次，全管线恰一次色调映射（详见 PostEffects）。
        */}
        <PostEffects />
      </Canvas>
    </div>
