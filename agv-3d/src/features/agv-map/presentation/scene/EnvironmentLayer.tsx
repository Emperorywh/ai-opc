 ...
 * - 地面/网格 geometry 与 material 各构建一次（useMemo），经 primitive 挂接、关闭 R3F 自动释放，
 *   由卸载 effect 统一 dispose；PMREM 由 LocalEnvironment 自行管理显式释放。
...
export function EnvironmentLayer({ bounds }: EnvironmentLayerProps) {
  ...
  // 地面/网格 geometry 与 material 确定性释放（SPEC §5.4）。
  useEffect(() => {
    return () => {
  ...
  }, [groundGeometry, groundMaterial, gridGeometry, gridMaterial])
  ...
  return (
    <>
      ...
      {/* SPEC §8.4 深色不透明地面（本期基线，反射地面属后续任务）；接收节点阴影，不投射。 */}
      <mesh
        position={[layout.center[0], 0, layout.center[1]]}
        rotation-x={-Math.PI / 2}
        receiveShadow
      >
        <primitive object={groundGeometry} attach="geometry" dispose={null} />
