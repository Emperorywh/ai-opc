 ...
 *
 * 场景与生命周期（TASK-009）：渲染数据包就绪（preparing/ready）后挂载 MapSceneView，
 * 呈现四类节点 InstancedMesh；场景视图在首帧渲染后驱动状态机 creating-scene → fading → ready，
 * 完成淡入后覆盖层卸载，露出完整场景。
 *
 ...
 */
function App() {
  const { state, controller } = useMapLoad()
  ...
  return (
    ...
    >
      <main className="agv-map-stage">
        {packet !== null && <MapSceneView packet={packet} controller={controller} />}
      </main>
