...
| 红线项 | 责任 TASK | 状态 | 说明 |
|---|---|---|---|
| 主 3D 图按真实位置呈现南海诸岛 / 九段线 | TASK-011（渲染层交付） | pass(自动+无头) | 主图与附图复用本 TASK 的同一份政治边界事实源（SPEC §3.8、§6）；运行时经共享红线扫描 `collectPoliticalRedLineGaps` 断言完整性，缺段 / 缺点按红线上报整页错误（不静默渲染残缺地图）；TASK-011 无头实测 16/16（10 段含台湾东侧段 26/26 采样命中 + 5 岛礁光点屏幕位置全部检出） |
| 右下 2D 南海附图 | TASK-012（渲染层交付） | pass(自动+无头) | 同上（同一契约单例 + 同一共享扫描）；TASK-012 无头实测 21/21（右下角定位、10 段虚线、5 岛礁点、5 标注齐全且 getBBox 实测不互叠）；TASK-016 最终无头验收再次确认（`acceptance-review-record.md` §3） |
| 审图号占位 | TASK-014（合规角标交付） | 占位已落地 / 取得审图号 pending | 页面左下角标常驻 `GS(202x)xxxx 号（待取得）`（不伪造已批复号码）+ SPEC §8 免责声明（逐字受测试保护）；发布前必须取得自然资源主管部门审图号并填入预留位；程序生成/拼装的地图公开发布前依法须送审 |
...
| 2026-07-30 | §2.5 状态更正：主 3D 图政治要素（TASK-011）、右下 2D 附图（TASK-012）、审图号占位（TASK-014）已由「未开始」更正为已交付并附无头验证锚点；SPEC §12 最终逐条验收与红线逐项确认见 `docs/acceptance-review-record.md`。人工逐点核对与审图号仍 pending（§4 不变）。 | TASK-016 worker |
