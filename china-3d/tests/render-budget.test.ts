/**
 * 4K 大屏渲染性能预算的不变量测试（TASK-015，SPEC §7.2 / §7.3 / §7.4）。
 *
 ...
 * 「逐帧分配被禁止」等不变量，无需启动浏览器 / WebGL（1080p / 4K 持续帧率是目标独显设备上的人工验收项，
 * 方法与记录见 docs/performance-measurement-record.md）。
 *
 * 覆盖（对应 TASK-015 验收条件 3、4）：
 * - DPR 上限 ≤ 2、下限 ≥ 1、下限 ≤ 上限（SPEC §7.3）；App 的 Canvas dpr 取自配置唯一源（源码扫描）。
...
/** 以 UTF-8 文本读取 src 下某个模块源码（App 装配接线断言用）。 */
function readSource(relativePath: string): string {
  return readFileSync(resolve(srcRoot, relativePath), 'utf-8')
}

describe('DPR 上限：生产渲染 DPR ≤ 2（SPEC §7.3、验收条件 3）', () => {
  it('dprMax = 2（不超过 2）', () => {
  ...
  it('App 的 Canvas dpr 取自 RENDER_BUDGET_CONFIG（唯一事实源，无硬编码 [1, 2] 字面量）', () => {
    const source = readSource('App.tsx')
    expect(source).toContain("from './config/render-budget'")
    expect(source).toContain('dpr={[RENDER_BUDGET_CONFIG.dprMin, RENDER_BUDGET_CONFIG.dprMax]}')
    // 无第二份 DPR 字面量装配（防止偷偷绕过配置层）。
    expect(source).not.toContain('dpr={[1, 2]}')
  })
})

describe('渲染目标尺寸：标准 1080p / 4K 档（SPEC §2 运行平台）', () => {
...
describe('显存预算：与 SPEC §7.2 量级一致且有限', () => {
...
describe('draw call 预算：结构性计数为正整数（SPEC §3.6 / §7.2）', () => {
  ...
  it('省界 draw call 预算 = 34（每行政区一个 drei Line，hover 可寻址）', () => {
  ...
  it('十段线 draw call 预算 = 12（每段一个 drei Line，含台湾东侧段独立审计）', () => {
...
describe('4096² 档位：显式可选、默认不启用、不自动升级（SPEC §7.2、验收条件 3）', () => {
...
describe('无运行时流式 / 自动低清 fallback（SPEC §7.3「无流式加载」）', () => {
...
describe('逐帧分配被禁止（SPEC §7.4「无运行时几何分配循环」）', () => {
