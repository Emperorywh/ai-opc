 ...
 * scripts/provinces 34 省目录视图（派生自契约层规范目录）与 src/geo-contracts 契约层。
 * 直接读取 public/geo 下已交付的生产省级边界资产（目录 + 几何 + provenance），证明
 * 34 省 / 港澳台 / 目录-几何双射 / 真值一致 / 环闭合 / 坐标范围 / 来源审计全部成立；
 * 并验证删台湾、重复标识等篡改路径被确定性发现。
 *
...
describe('资产级篡改确定性失败', () => {
  ...
  it('删除香港 / 澳门目录条目后政治红线锚点命中', () => {
    const tampered = cloneDirectory()
    tampered.entries = (tampered.entries as Array<{ id: string }>).filter(
      (e) => e.id !== 'CN-810000' && e.id !== 'CN-820000',
    )
    const outcome = verifyProvincesAsset({
      directory: tampered,
      geometry: asset.geometry,
    ...
    const codes = outcome.errors.map((e) => e.code)
    expect(codes).toContain('provinces-asset.missing-political-id')
    expect(outcome.errors.some((e) => e.message.includes('CN-810000'))).toBe(true)
    expect(outcome.errors.some((e) => e.message.includes('CN-820000'))).toBe(true)
  })
...
describe('契约层接纳内环（带洞多边形）结构', () => {
  /**
   * DataV 生产数据恰好无内环（中国省级边界无飞地洞），但契约层与深度校验都必须正确处理内环——
   * 争议区修正等带洞多边形已在政治边界资产中出现。此用例证明：
