/**
 * 共享运行时高程查询测试（TASK-006 验收 4：CPU 端采样器）。
 *
 ...
 *
 * 覆盖：
 * - 小型 2×2 / 已知编码夹具在像元中心、边界、四角中心插值处得到可计算的真实米制结果。
 * - 生产资产抽样，CPU 查询值与 TASK-003 资产验证的地势相对关系一致，浅水点保持负值。
 * - 重复创建多个消费者，底层 16 位数据只解码一次；release 后不再持有缓存。
 * - 损坏位深、错配元数据、范围外坐标、非有限输入显式失败，不返回伪造海拔。
 */
...
describe('查询异常路径：越界 / 非有限 / 已释放（不返回伪造海拔）', () => {
...
describe('queryAtLonLat · 经纬度采样路径', () => {
  ...
  /** 投影成功值（夹具坐标必然合法）。 */
  function mercatorOf(lon: number, lat: number): { readonly x: number; readonly y: number } {
    const r = projectToMercator(lon, lat)
    expect(r.ok).toBe(true)
    return (r as { readonly value: { readonly x: number; readonly y: number } }).value
  }
  ...
  it('范围中心经纬度映射到对应 UV，与 UV 路径结果一致（墨卡托纬度非线性，中心 lat 不映射到 v=0.5）', () => {
    ...
    const lon = 102
    const lat = 22
    const target = mercatorOf(lon, lat)
    const sw = mercatorOf(extent.west, extent.south)
    const ne = mercatorOf(extent.east, extent.north)
...
describe('queryAtWorld · 世界坐标采样路径（hover 反查语义）', () => {
  ...
  it('世界坐标查询与经纬度查询结果一致（同一位置两条路径给出同一海拔）', () => {
    ...
    const w = (world as { readonly value: { readonly x: number; readonly z: number } }).value
    const byWorld = expectQueryOk(provider.queryAtWorld(w.x, w.z))
...
describe('生产资产抽样（CPU 查询与 TASK-003 资产验证一致）', () => {
  /**
