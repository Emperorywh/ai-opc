/**
 * 地形渲染配置与配置不变量（TASK-006）。
 *
 * 角色与依赖方向：
 * - 本模块属于运行时配置层（src/config），是「垂直夸张系数 k」与「地形网格分段预算」的**唯一**
 *   权威。渲染层（src/three/ChinaTerrainMesh）、页面装配（src/App）、自动化测试都只能通过本模块
 *   取得合法配置——禁止在组件内自行硬编码 k 或分段数，也禁止在别处另写一套校验
 *   （SPEC §3.2、§7.1、§7.2）。
 * - 单向依赖：本模块不依赖 React、R3F、Three.js 或任何 DOM/场景对象（纯 TS）。这使得本模块
 *   可在 Node 测试环境直接断言「非法配置被拒绝」「生产默认未被偷偷改低」等配置不变量。
 *
 * 垂直夸张 k（SPEC §3.2、§13）：
 * - 真实海拔 h（米）→ 世界 y：y = h·k。k 在 GPU vertex shader 中应用（见 terrain-shaders.ts），
 ...
 * - **绝对禁止** CPU 为高精网格逐顶点采样并写入 position（SPEC §7.1 红线）；分段只决定 GPU 顶点
 *   密度，不决定「是否走 GPU 位移」——位移恒在 shader 内。
 * - 下限 1：保留极低分段用于自动化/低资源测试环境（如 64²），但**生产默认必须保持 2048²**，测试
 *   配置与生产配置边界清楚分离（本模块分别导出 PRODUCTION_TERRAIN_CONFIG 与 TEST_TERRAIN_CONFIG）。
 *
 * CPU/GPU 解码一致性：
 * - 着色器内的解码公式（见 terrain-shaders.ts 的 decodeElevationMeters）与本模块的
 *   decodeNormalizedToElevation 是同一仿射：h = normalized · (max − min) + min。min/max 来自经契约
 *   校验的 heightmap 元数据（src/geo-contracts decodeUint16ToElevation 的唯一源），由配置层透传给
 *   着色器 uniform——不存在第二套解码常量。位移 = decode(normalized) · k，**只**用于世界 y，不参与
 *   任何颜色映射（分层设色在片元着色器内按真实 h 重新采样，见 SPEC §3.1）。
 */
 ...
 *
 * 成功返回的配置对象已 Object.freeze，消费者不应（也无法）就地修改——改配置应重新调用
 * resolveTerrainConfig，使配置变化在渲染层走受控路径（uniform 更新），杜绝隐式状态。
 */
 ...
 *
 * 仅用于自动化环境（Node 测试无 GPU、CI 低显存、无头软件渲染）下驱动渲染层装配而不爆顶点内存。
 * 生产默认值不得被本配置污染——两条路径分别导出，边界清楚。
 */
 ...
 *
 * 这是 vertex shader 内 `displaced.z += h · uExaggeration · uRise`（uRise=1 时）的 CPU 侧镜像，
 * 供测试断言「y 使用真实海拔乘夸张系数」。改 k 只改 y，不改 h（真实高程解码）也不改平面位置——
 * k 的作用边界由此函数显式表达。
 */
