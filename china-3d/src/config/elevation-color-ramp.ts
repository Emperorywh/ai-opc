/**
 * 地表分层设色色阶——唯一事实源（TASK-006）。
 *
 * 角色与依赖方向：
 * - 本模块属于运行时配置层（src/config），是「真实米制海拔 → 颜色」的**唯一**权威。GPU 片元着色器
 *   （src/three/terrain-shaders）、自动化测试、以及 TASK-014 的海拔图例都只能通过本模块取得断点、
 *   基线色与 256×1 ramp——禁止在着色器、测试或图例里各自复制一份断点或颜色。本模块单向依赖
 *   TypeScript 自身，不依赖 React / R3F / Three.js 或任何 UI 组件——材质层与图例层都单向消费
 *   本模块，互不反向依赖。
 *
 * 真实海拔查色（SPEC §3.1）：
 * - 颜色只由「真实米制海拔 h」决定，与垂直夸张系数 k 无关——k 只放大世界 y（§3.2），不进入色阶。
 *   故同一真实高程在 k=1.5/2.0/3.0 下颜色完全一致。**绝不**用位移后的 world-y 查色：world-y 已被
 *   k 放大，用它查色会让整图颜色偏移 k 倍（SPEC §3.1、§7.1）。
 ...
 *
 * 插值策略（SPEC §3.1「断点处可做平滑插值或硬切」——本实现选定平滑插值且全仓唯一）：
 ...
 * - 明度取向：基线色整体偏暗（SPEC §3.1「色阶整体略压低明度」），靠后续边界线发光与水面反光
 *   提亮，避免深色背景下糊成一团。
 */
 ...
 *
 * 各行：区间（地貌）→ 基线色。控制点海拔取区间下界；末两行（5000m 雪线、9000m 上限）
 ...
 * DataTexture 与着色器 uniform；TASK-014 图例层直接消费 ELEVATION_COLOR_BREAKPOINTS 与本域
 ...
 *
 * 这是「错误 minH/maxH 被拒绝」的确定性失败点：若元数据上下限偏离 ELEVATION_COLOR_DOMAIN
 * （如缺失水下区间的 [0, 9000]、或上限偏移的 [−1500, 10000]），着色器归一化会使断点颜色
 * 错位（偏色），故在此显式抛 elevation-color.domain-mismatch，绝不静默放行。
 *
