/**
 * 政治边界补充要素（十段线 + 岛礁点位）主图呈现的视觉与几何配置——唯一事实源（TASK-011，SPEC §5.3/§6）。
 *
 * 角色与依赖方向：
 ...
 *   唯一源，用来把 densify 间距与点位半径表达成主图尺度的分数）、同层 src/config/sea-surface
 ...
 *   等不变量。
 *
 * 与省界配置（src/config/province-borders）的分工与共享（SPEC §3.6 省界 / §5.3 九段线）：
 * - 共享：densify 间距口径（= 主图世界宽度 / 4096，与 heightmap 纹素分辨率一一对应）、NDC 深度偏移
 *   口径（Line2 在地图尺度下抗 z-fighting 的同一种结构性手段，由 src/three/line-depth-bias 的同一
 *   注入函数应用）。两处各自定义同值常量而非跨配置 import——二者是相互独立的渲染层，同值反映同一类
 *   问题在同一地图尺度下的同一解，任一层调整不应隐式波及另一层。
 * - 区分（SPEC §5.3「样式与省界区分（如更亮的发光虚线）」）：十段线基线色取暖琥珀（#ffd180），与省界
 *   浅青白（#9fe8d8）冷暖相对、色相分明；线宽略粗（2.0 vs 1.6 px）；并启用虚线节拍（dashSize /
 *   gapSize），使十段线在视觉上「更亮、暖色、虚线」，与省界「轻发光、冷色、实线」明确区分，不混淆。
 *
 * 海平面贴合（SPEC §3.5 海面 / §5.3 九段线贴地或海面）：
 * - 十段线绝大部段与岛礁点位落在海域（九段线本就是海域主张线，曾母暗沙等是水下礁滩）。若照省界公式
 *   world_y = h·k + epsilon 直接贴合地形，海域负高程（h<0，水下大陆架）会把线 / 点压到海面（y=0）
 *   之下，被半透明海面（depthWrite=false，但其片元仍参与透明混合）与水下地形在视觉上吞没。
 * - 故政治要素采用「海平面贴合」语义：world_y = max(h·k, SEA_LEVEL_Y_METERS) + epsilon。陆地
 *   （h·k>0）贴合真实地形（与省界一致）；海域（h·k≤0）钳制到海平面 y=0 之上 epsilon，使线 / 点
 *   恒位于半透明海面之上、可见。海平面 y 取自 SEA_LEVEL_Y_METERS（=0，与动态海面同一米制海平面，
 *   非「与地图脱节的固定世界坐标」），高程 h 取自共享 ElevationProvider（与地形 GPU 位移同一份高程
 *   事实源）——二者共同构成「共享高程 / 海平面语义」。
 *
 * 非官方审图限制（SPEC §6 / §8 / §13）：
 * - 本配置只决定「如何画线 / 画点」（颜色 / 线宽 / 虚线 / 半径 / 深度偏移），不承载也不校验坐标——
 *   坐标唯一事实源是 TASK-004 的政治边界补充资产（public/geo/china-political-boundary.json）。本配置
 *   与渲染层都不得复制、手改或在组件内补写十段线 / 岛礁坐标。
 * - 本配置与渲染层的呈现属于「内部展示状态」：政治边界补充数据为非官方审图数据
 *   （isOfficialSurvey=false），九段线几何顶点与争议区边界的国标逐点一致性以人工核对为准
 *   （docs/political-review-record.md），公开发布前必须取得自然资源主管部门审图号。本 TASK 不通过
 *   任何视觉手段（如加审图号角标）宣称已审图。
 */
 ...
 *
 * 与省界 PROVINCE_BORDER_DENSIFY_SPACING_METERS 同公式、同值（≈ 1742 m，落在 SPEC §3.6 的 1–2 km
 * 区间）。沿世界 XZ 平面弧长度量，使十段线全境 densify 密度与省界一致。冻结派生自主图世界包围盒
 * （唯一源），不写死绝对米数。
 */
...
/**
 * 十段线基线色（SPEC §5.3「更亮的发光虚线」与省界区分，十六进制）。
 *
...
/**
 * 岛礁 / 附属岛屿点位基线色（SPEC §5.3 / §6 岛礁点位在真实位置有可见标记，十六进制）。
 *
 * 取 #ffe0a0（r=255、g=224、b=160）：与十段线同色系的更亮暖琥珀，使点位与十段线在视觉上归为同一
 * 「政治边界补充要素」族，又比线更亮以突出「点位」。恒定基线，不随岛礁 / 高程变化。点位规范名称
 * （钓鱼岛 / 赤尾屿 / 曾母暗沙等）的文本标注由后续统一标签任务呈现，本 TASK 只画可见标记光点。
 */
 ...
 * 十段线基线色·字节 RGB·线宽·虚线节拍 / 岛礁点位基线色·字节 RGB·半径全部在此，不存在第二套政治要素
 * 常量。冻结防止运行时被偷偷改（如把 epsilon 改 0 会让海域线段沉到海面之下被吞没、把海平面 y 改非 0
 * 会与动态海面脱节），任何调整都必须改本模块并同步测试。
 */
