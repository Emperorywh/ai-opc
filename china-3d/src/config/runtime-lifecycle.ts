/**
 * 大屏长时运行生命周期的运行时参数——唯一事实源（TASK-015，SPEC §7.4）。
 *
 * 角色与依赖方向：
 * - 本模块属于运行时配置层（src/config），是「context 丢失后等待恢复的超时、resize 防抖窗口」的**唯一**
 *   权威。运行时控制器（src/three/RuntimeLifecycleController）、自动化测试都只能通过本模块取得这些
 *   参数——禁止在控制器 / 测试里各自复制一份超时或防抖常量。
 * - 单向依赖：本模块不依赖 React / R3F / Three.js / DOM（纯数值常量），故自动化测试可在 Node 环境
 *   直接断言「防抖窗口为正有限」「超时为正有限」等不变量。
 *
 * context 丢失后等待恢复的超时（SPEC §7.4「丢失时暂停渲染、恢复时重建」的防线）：
 * - context-lost 阶段下，若浏览器在超时内未触发 webglcontextrestored，控制器把状态迁移到
 *   restore-failed 并附「context 恢复超时」诊断——这使「context 丢失后永不恢复」的场景有显式终态
 *   而非无限空白等待。
 * - 取 8 秒：远长于浏览器常规 context 恢复延迟（通常 < 1 秒），又不过长（大屏运维能及时看到诊断）。
 *
 * resize 防抖窗口（SPEC §7.4「窗口变化 debounce 后再更新 camera/renderer 尺寸」）：
 ...
 *
 * 超时后控制器迁移到 restore-failed（显式终态 + 诊断），避免 context-lost 无限等待。8 秒远长于
 * 浏览器常规恢复延迟，正常恢复路径不会误触超时。
 */
 ...
 * 运行时生命周期参数（冻结）。
 *
 * 控制器与测试共享的同一份事实源：context 恢复超时 + resize 防抖窗口。冻结防止运行时被偷偷改
 * （如把防抖改 0 会退化为逐次提交造成更新风暴、把超时改 0 会误把正常恢复判为失败），任何调整都
 * 必须改本模块并同步测试。
 */
