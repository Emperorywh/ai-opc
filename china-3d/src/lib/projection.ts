 ...
 *   d3-geo（geoMercator，SPEC §3.3、§10），不依赖 React、R3F、Three.js 场景对象或任何
 *   UI 状态。渲染/交互层单向依赖本模块，反向依赖即违规。
 *
 ...
 *   定义常数），与 d3-geo geoMercator、离线 DEM 重投影（scripts/dem/）一致。
 ...
 *   hover 命中地形后用 invertWorld 把世界坐标还原成经纬度，再由上游拾取逻辑判定所属
 *   省份——反查只忠实还原坐标，不做范围/省份裁剪。
 *
 ...
 *
 * 地形网格用本包围盒确定 PlaneGeometry 的世界尺寸与定位，使 mesh 覆盖范围与统一投影完全
 * 一致；heightmap 的行 0=北、列 0=西由地形 mesh 据本契约对齐 UV（u 随 +X/东增，
 * 纹理 v 对应北→南行序）。
 *
 ...
 * 映射不同。SVG overlay 消费时自行把 v 翻转成屏幕 y。
 *
