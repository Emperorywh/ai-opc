/**
 * 标签地形遮挡判定（领域层，TASK-010，SPEC §7.5）。
 *
 * 角色与依赖方向：
 * - 本模块属于运行时访问层（src/lib），与 place-labels.ts / elevation.ts 同层。它把「标签世界
 *   坐标 + 相机世界坐标 + 地形世界 y 采样器」确定性地变换为「标签相对相机是否被前方地形遮挡」
 *   的可见性状态（visible / occluded / indeterminate），供渲染层（src/three/PlaceLabels）只消费、
 *   不再自行实现射线行进或距离比较（遮挡判断能力与标签渲染解耦）。
 * - 单向依赖：本模块**不**依赖 React / R3F / Three.js / DOM / hover 状态 / 资产数据——它是纯
 *   函数，只接收坐标与一个抽象采样器闭包（TerrainWorldYSampler）。这使遮挡判定可在 Node 环境
 *   （vitest）用确定性几何夹具完整覆盖「无遮挡 / 前方山体遮挡 / 命中点位于标签之后 / 射线擦边 /
 *   相机移动」等场景，无需启动 WebGL / 浏览器（视觉验收由 pnpm dev 无头渲染承担）。
 *
 * 与 Billboard 朝向正交（不得通过关闭深度测试让标签永久穿透地形）：
 * - Billboard 让文本恒面向相机（渲染层职责），解决「被自身朝向遮挡」。本模块解决「被前方地形
 *   遮挡」——二者正交：本判定只关心「标签→相机」连线是否被地形抬起打断，与文本朝向无关。渲染
 *   层据此调制标签透明度（fillOpacity），深度测试保持开启（既不关闭深度测试让标签永久穿透，
 *   也通过淡化避免被前方山体硬切时整块标签突兀消失的违和感）。
 *
 * 高度场射线行进（SPEC §7.5「对每个标签做一次 raycast，标签位置→相机，命中地形且命中点更近
 * 则降低透明度」）：
 * - 把「标签→相机」连线参数化为 P(t) = label + t·(camera − label)，t∈[0,1]。在 t∈(near, far)
 *   的内部区间均匀取 maxSamples 个采样点（跳过标签端 nearMargin 与相机端 farMargin，避免采到
 *   标签自身锚点地形或相机贴地点）。
 * - 每个采样点取其世界 (x, z) 查地形世界 y = h·k（由采样器闭包提供，与 GPU 位移同一高程事实
 *   源经夸张系数 k 还原，故判定与实际地形起伏一致）。
 * - 距离比较：若地形 y 高出「射线 y」超过 verticalClearance，则该处地形挡在标签与相机之间 →
 *   occluded（命中点比标签更近相机）。verticalClearance 提供抗擦边抖动的余量（地形需「明显」
 *   高过视线才算遮挡，擦边不计），避免采样落在山脊正上方时因亚采样 / 浮点抖动造成的可见性
 *   来回翻转。
 * - 任一采样点遮挡即整体 occluded（短路返回）；全部采样点都未遮挡 → visible。
 *
 * 不确定状态与生命周期（字体加载未完成、地形不可用或标签已卸载时有明确生命周期处理，不产生
 * 错误射线或僵尸更新）：
 * - 退化射线（标签与相机重合 / 非有限）→ indeterminate（无法判定，不伪造可见 / 遮挡）。
 * - 射线过短（nearMargin + farMargin ≥ 射线长，无可采样内部区间）→ indeterminate。
 * - 全部采样点查询失败（地形不可用 / 越出元数据范围 / provider 已释放 / 反投影失败 → 采样器
 *   返回 null）→ indeterminate（不伪造结果）。渲染层据此「保持当前透明度」，既不伪造遮挡淡化
 *   已可见标签，也不在偶发查询失败时抖动。这与 elevation「不以魔法 0 混淆异常与海平面」同构：
 *   错误显式区分，绝不伪装成成功读数。
 *
 * 无分配约束（不为每次检查重复创建大对象、不造成逐帧抖动或分配压力）：
 * - 本函数全程不 new 数组 / 对象：循环用 number 局部量、短路返回字符串字面量。可被渲染层在帧
 *   循环中高频调用（每 N 帧一次 × 34 个标签）而不产生 GC 压力。状态（目标 / 当前透明度数组）
 *   由渲染层在挂载期一次性分配并复用，本函数只读输入、只返回字面量。
 */
 ...
 *
 * 由渲染 / 装配层从共享 ElevationProvider + 夸张系数 k 构造（见 App 的 PlaceLabelsLayer）：
 *   (x, z) => { const q = provider.queryAtWorld(x, z); return q.ok ? q.meters * k : null }
 * 本模块不持有 provider 引用、不复制高程数据——采样器是注入的纯查询闭包，使本函数可在 Node
 * 环境用合成采样器（常数 / 阶梯峰）完整覆盖各遮挡场景，无需真实 heightmap。
 */
...
export interface LabelOcclusionInput {
  ...
  /** 地形世界 y 采样器（抽象闭包，由装配层从共享 ElevationProvider + k 构造）。 */
 ...
 *
 * 射线方向：从标签指向相机（camera − label）。我们在其内部区间均匀采样、对每个采样点比较地形
 * 世界 y 与射线 y：地形明显高过视线（超出 verticalClearance）即遮挡。短路返回，全程无分配。
 *
 * @returns 'occluded' 任一采样点被前方地形挡住；'visible' 全部采样点未被挡；'indeterminate'
 * 退化 / 全失败。
 */
export function computeLabelVisibility(
  ...
  // 在 (tStart, tEnd) 内均匀取 sampleCount 个内部点（i = 1..sampleCount，分母 sampleCount+1，跳过
  // 两端）。采样点数固定上限、沿区间均匀分布——确定性策略，无随机抽样（不得用随机抽样造成闪烁）。
  for (let i = 1; i <= sampleCount; i++) {
    ...
    anySampled = true
    // 距离比较：地形世界 y 高出「射线在该处的 y」超过 verticalClearance → 该处地形挡在标签与
    // 相机之间（命中点比标签更近相机）→ 遮挡。短路返回，不再采样后续点。
