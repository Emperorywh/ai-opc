/**
 * 政治边界补充要素（十段线 + 岛礁点位）的主图呈现数据准备（领域层，TASK-011，SPEC §5.3/§6）。
 *
 * 角色与依赖方向：
 * - 本模块属于运行时访问层（src/lib），与 province-borders.ts / elevation.ts / projection.ts 同层。它把
 *   「政治边界补充契约（PoliticalBoundaryContract，TASK-004 共享事实源）+ 共享双线性高程查询
 *   （ElevationProvider）+ 夸张系数 k + densify / 海平面贴合 epsilon 配置」确定性地变换为「十段线各段的
 *   densify 海平面贴合子段端点 + 岛礁点位的世界坐标」，供渲染层（src/three/PoliticalFeatures）**只消费、
 *   不再计算**。本模块是 SPEC §5.3「九段线作为独立 LineString 渲染（贴地 / 海面）」+ §6「南海诸岛 /
 *   钓鱼岛 / 赤尾屿等必需点位」的主图呈现的领域实现。
 ...
 *   EXPECTED_NINE_DASH_SEGMENT_COUNT / TAIWAN_EAST_SEGMENT_INDEX）、同层坐标权威 src/lib/projection
 *   （projectToWorld——lon/lat→世界 x,z 的唯一入口）、同层红线扫描单源 src/lib/political-red-line
 *   （collectPoliticalRedLineGaps）、同层高程权威 src/lib/elevation（ElevationProvider.queryAtWorld——
 *   世界点→真实米制海拔的唯一入口）。**禁止**依赖 React / R3F / Three.js / DOM / hover 状态 / src/config
 *   （与省界准备层同构的分层约束）。
 *
 * 唯一事实源（SPEC §5.3 / §5.4：主图与 2D 附图复用同一份数据，不得复制、手改或在组件内补写坐标）：
 ...
 *   任何段或点，**不**从别处读取九段线 / 岛礁数据。后续 2D 南海附图（TASK-012）复用同一契约。
 *
 * 红线完整性（SPEC §6：十个线段和全部必需点位都被主图渲染模型消费；删台湾东侧段或一个必需附属岛屿 →
 * 渲染准备失败，不能静默显示残缺地图；不把十段线合并为不可核查的单条连续折线，不得遗漏台湾东侧段）：
 * - 准备入口对 SPEC §6 红线点名项做独立锚点断言（与 scripts/verify-assets/political-deep 的资产级深度校验
 *   共用同一份领域真值：红线「目录」唯一来自契约层 political-catalog，红线「扫描逻辑」唯一来自
 *   src/lib/political-red-line 的 collectPoliticalRedLineGaps——不在本模块手写第二套段数 / 段序号 /
 *   岛礁名清单或扫描代码）：
 *   · 恰好 EXPECTED_NINE_DASH_SEGMENT_COUNT（10）段，段序号 1..10 全在；
 *   · 台湾东侧段（TAIWAN_EAST_SEGMENT_INDEX = 10）独立硬编码锚点（不经段序号清单间接得出，即便清单被
 *     误改仍要求资产含此段）；
 *   · 点名岛礁（钓鱼岛 / 赤尾屿 / 曾母暗沙）均在。
 ...
 *
 * 海平面贴合（SPEC §3.5 海面 / §5.3 九段线贴地或海面：高度由共享高程 / 海平面语义确定，既不被海面完全
 * 吞没，也不使用与地图脱节的固定世界坐标）：
 ...
 *   seaLevelYMeters 取自 src/config/political-features（= SEA_LEVEL_Y_METERS = 0，与动态海面同一米制
 *   海平面）。二者构成「共享高程 / 海平面语义」，不使用与地图脱节的固定世界坐标。
 * - h·k 与 GPU vertex shader 的 `displaced.z += h * uExaggeration * uRise`（src/three/terrain-shaders）、
 *   省界准备的 queryTerrainWorldY 是同一公式；此处内联 h·k 以避免 src/lib 反向依赖 src/config（被禁止的
 *   是复制 heightmap 解码 / 投影公式，本模块全部复用 projection / elevation 的唯一入口）。
 *
 ...
 * - 十段线绝大部段在海域（海面平坦），densify 主要保证跨岛礁 / 跨陆海交界处逐顶点贴合，而非追随海底
 *   起伏。
 *
 * 异常语义（与省界准备同构：无效输入或查询失败时整条准备明确失败）：
 ...
 * 可独立定位，且渲染层每段一个 Line（draw call 可审计）。
 */
 ...
 *
 * h·k 与 GPU vertex shader 的 `displaced.z += h * uExaggeration * uRise`（src/three/terrain-shaders）、
 * 省界准备的 queryTerrainWorldY 是同一公式；此处内联以避免 src/lib 反向依赖 src/config。
 */
...
/**
 * 对政治边界契约做红线完整性断言（SPEC §6，与资产级深度校验、与后续 2D 南海附图准备共用同一份领域真值）。
 *
 * 任一红线缺项 → 抛 PoliticalFeaturePrepError（稳定 code），整条准备失败、不产出残缺十段线 / 缺失岛礁。
 * 断言项：
 * - 恰好 EXPECTED_NINE_DASH_SEGMENT_COUNT（10）段，段序号 1..10 全在；
 * - 台湾东侧段（TAIWAN_EAST_SEGMENT_INDEX = 10）独立硬编码锚点；
 * - 点名岛礁（钓鱼岛 / 赤尾屿 / 曾母暗沙，契约层 REQUIRED_ISLAND_NAMES）均在。
 *
 * 数据复用（SPEC §5.4「主图和附图共享同一份数据」）：红线「目录」唯一来自契约层 political-catalog，
 * 「扫描逻辑」唯一来自共享单源 collectPoliticalRedLineGaps（src/lib/political-red-line，TASK-004 交付）。
 * 本函数只据缺项事实按既定顺序抛本层稳定错误码（political-features.*），不再各自遍历 features / 对照
 * 目录常量——不存在第二套段数 / 段序号 / 岛礁名清单或扫描代码。
 */
 ...
 *
 * 争议区修正（disputedBoundaryCorrection）不被本模块消费——主图只呈现十段线与岛礁点位，争议区按中国
 * 主张画法的修正已体现在 TASK-004 省级边界资产的省界表达中；争议区完整性的运行时把关由资产校验管线
 * （scripts/verify-assets/political-deep）承担。
 *
 * @param contract 政治边界补充契约（TASK-004 共享事实源，已通过 political-boundary 契约校验）。
