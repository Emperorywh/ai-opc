/**
 * 政治边界补充数据的运行时加载与契约校验（数据访问层，TASK-011，SPEC §5.3/§6）。
 *
 * 角色与依赖方向：
 * - 本模块属于运行时访问层（src/lib），与 province-geometry.ts / elevation.ts / projection.ts 同层。
 *   它把 TASK-004 交付的静态资产 public/geo/china-political-boundary.json fetch 进来、按
 *   political-boundary 契约校验，返回类型安全的 PoliticalBoundaryContract（features 数组直接供
 *   src/lib/political-features 的主图渲染准备纯函数消费）。本模块只依赖契约层 src/geo-contracts
 *   （validatePoliticalBoundary——唯一校验入口、PoliticalBoundaryContract 类型）。不依赖 React /
 *   R3F / Three.js / DOM（fetch 是 Web 标准 API）。
 *
 * 单一事实源（SPEC §5.3 / §5.4：3D 主图与 2D 南海附图复用同一份事实源，不维护两套坐标）：
 * - 本模块是主图运行时消费政治边界数据的**唯一**取数入口：fetch
 *   public/geo/china-political-boundary.json。十段线 / 岛礁坐标全部来自该资产，组件层
 *   （src/three/PoliticalFeatures）不得自行 fetch 或硬编码坐标。
 * - 后续 2D 南海附图（TASK-012）复用同一资产（SPEC §3.8、§5.4），不维护第二套坐标。
 *
 * 失败语义（与 loadProvinceGeometry 同一「绝不静默退化」原则；SPEC §6 政治边界完整性是事故级红线）：
 * - fetch 失败（网络 / HTTP 非 2xx）→ 抛 PoliticalBoundaryLoadError（fetch-failed）。
 * - 资产未通过 political-boundary 契约校验（kind / crs / features 结构 / 段序号唯一 / 岛礁名非空 /
 *   坐标合法性 / source.sourceId 非空等）→ 抛 PoliticalBoundaryLoadError（contract-invalid）。绝不
 *   返回部分 / 伪造政治边界——主图渲染准备层会基于残缺数据产出残缺十段线 / 缺失岛礁，违反政治边界
 *   完整性红线（SPEC §6）。
 * - 红线完整性（恰好 10 段含台湾东侧段、点名岛礁在、点名争议区在、坐标落在中国主图、来源非官方审图）
 *   由资产级深度校验（scripts/verify-assets/political-deep）在离线管线把关；运行时由准备层
 *   （src/lib/political-features 的 preparePoliticalFeatures）对「十段 + 台湾东侧段 + 点名岛礁」做
 *   独立锚点断言，缺任一项即抛错阻断渲染。
 * - 校验复用契约层唯一入口 validatePoliticalBoundary，不在本层另写一套政治边界校验。
 */
 ...
 * validatePoliticalBoundary 做契约校验（TASK-004 已通过政治边界深度校验的资产，运行时再经同一入口
 * 作为防线）。任一步失败抛 PoliticalBoundaryLoadError。
 *
 * 该函数只在浏览器运行（用 fetch）；测试请直接构造 PoliticalBoundaryContract 字面量喂给
 * preparePoliticalFeatures，或以 stub 的 fetch 驱动本函数。
 */
