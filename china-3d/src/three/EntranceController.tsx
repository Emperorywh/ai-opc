/**
 * 入场状态机的单一帧循环驱动器（渲染层，TASK-013，SPEC §4.3）。
 *
 * 角色与依赖方向：
 * - 本组件属于渲染层（src/three），挂在 Canvas 内，是「加载 / 入场编排」的**唯一**时间源与状态
 *   驱动器。它每帧从 R3F 共享 clock（state.clock.getElapsedTime()）派生入场 elapsed、调用领域层纯
 *   函数（src/lib/entrance-state）派生当前阶段，把「阶段 + elapsed」写入共享入场帧 ref（供各渲染层
 *   useFrame 只读消费），并在阶段切换时回调上层（App）以驱动 DOM 加载反馈与相机交互锁。
 * - 本组件**只**依赖：领域层（entrance-state 的纯函数 + 类型）、配置层（ENTRANCE_DURATIONS 冻结
 *   时序）、R3F useFrame。**不**持有资产数据、不复制时序常量、不维护第二份 clock / 计时器 / 布尔
 *   组合（SPEC §7.4「动画时钟统一」「无运行时分配循环」）。
 *
 * 统一时间源与幂等起始捕获（StrictMode 重挂载 / 资产完成顺序变化不会导致动画多次启动或提前解锁）：
 * - 入场 elapsed = R3F 共享 clock − 起始时刻。起始时刻在「资产全部就绪且无失败」的**首帧**幂等捕获
 *   一次（startClockRef.current === null 时才写），之后不再改写。StrictMode 重挂载不重建 ref（同一
 *   fiber 同一 ref 对象），故起始时刻只捕获一次、动画只启动一次；资产完成顺序变化只影响「何时
 *   ready」，不影响「ready 后单调递增的 elapsed」——阶段顺序固定、每阶段只进一次。
 * - 无 new THREE.Clock()、无 setInterval / setTimeout：唯一时钟是 R3F 的共享 clock（与海面波动
 *   SeaSurface、标签遮挡 PlaceLabels 的 useFrame 同源），无独立漂移时钟。
 *
 * 运行时暂停（TASK-015 集中编排，SPEC §7.4）：注入共享 runtimeFrame 时，本组件每帧先检查
 *   runtimeFrame.paused。context-lost / restoring 期间 paused=true，本组件冻结入场 elapsed（不派生 /
 *   不写 entranceFrame，保留暂停前最后一帧），并把暂停时长折叠进起始时刻偏移，使恢复后入场从原位
 *   继续、无跳变。本组件**不**监听 context 事件——paused 由 RuntimeLifecycleController 单点写入
 *   runtimeFrame，本组件只读消费（集中编排契约）。
 *
 * 阶段切换回调（驱动 DOM 与相机锁，非每帧）：
 * - 阶段切换约 4 次（loading→terrain-rise→labels-fade-in→scene-layers-fade-in→interactive，或
 *   loading→error），仅在切换帧回调 onPhaseChange，使 App 以普通 React state 更新 DOM 加载反馈与
 *   相机 enabled——不是每帧 setState（避免 60fps 全场景重渲染）。各渲染层的逐帧 rise / 透明度由它们
 *   各自 useFrame 读共享入场帧直接写入材质 uniform / opacity，不经过 React state。
 *
 * 失败终态（加载失败显式终止状态机并保留诊断，不自动回退）：
 * - readiness.failed=true 时，起始时刻永不捕获（条件含 !failed），elapsed 恒 0，deriveEntrancePhase
 *   返回 error 终态。各渲染层 rise=0 / 透明度=0（elapsed=0），入场动画不继续；DOM 由 App 的红线
 *   整页错误通道显示可诊断错误信息、相机保持锁定。绝不退化为平面 / 缺层地图。
 */
...
export interface EntranceControllerProps {
  /** 受跟踪资产的聚合就绪状态（由 App 从四个资产 hook 派生）。 */
  ...
  /**
   * 共享运行时帧（TASK-015 集中编排，SPEC §7.4）。注入时本组件每帧先检查 runtimeFrame.paused：
   * context-lost / restoring 期间冻结入场 elapsed（不推进视觉状态），并把暂停时长折叠进起始时刻
   * 偏移，使恢复后入场从原位继续、无跳变。未注入时不检查暂停、入场始终推进。
   */
 ...
 *
 * 挂载后每帧：幂等捕获起始时刻 → 派生 elapsed → deriveEntrancePhase 得当前阶段 → 原地写共享入场帧 →
...
export function EntranceController({
  ...
  // 运行时暂停追踪（TASK-015）：暂停开始时刻（R3F 共享 clock 秒）；null=未暂停。
  ...
  useFrame((state) => {
    ...
    // 运行时暂停（TASK-015 集中编排）：context-lost / restoring 期间冻结入场推进。注入 runtimeFrame 时
    // 检查 paused：进入暂停记录起始（仅一次）；暂停期间直接 return（不派生 / 不写 entranceFrame），
    // entranceFrame 保留暂停前最后一帧的值——各渲染层据此保持冻结画面（地形 rise / 标签 / 水面边界
    // 透明度不变）。
    ...
    // 不替换整对象——entranceFrame.current 自始至终是 App useRef 创建的那一个，跨帧引用稳定。
