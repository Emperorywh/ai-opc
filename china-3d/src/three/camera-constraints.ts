/**
 * 受约束东南斜俯视相机的纯计算契约（TASK-008，SPEC §4.1）。
 *
 * 角色与依赖方向：
 * - 本模块属于渲染层的纯计算子层（src/three），但**不**依赖 three / React / R3F / DOM——只依赖
 *   坐标层 src/lib/projection（MAIN_MAP_WORLD_BOUNDS —— 主图世界米制包围盒的唯一源）、渲染派生
 *   常量 src/three/terrain-layout（TERRAIN_PLANE_LAYOUT.centerZ —— 主图世界 z 中心）、配置层
 *   src/config/terrain-config（TERRAIN_EXAGGERATION_MAX —— 夸张系数上限的唯一源）与契约层
 *   src/geo-contracts（CHINA_TERRAIN_ELEVATION_ENCODING.maxValueMeters —— 高程编码上限的唯一源）。
 *   所有相机约束（最近/最远距离、最大极角、target 边界、FOV、near/far）与默认机位都由这几份
 *   几何 / 配置事实派生，没有任何魔法绝对坐标。这让自动化测试可在 Node 环境直接断言「约束只随
 *   地图包围盒与地形尺度变化，与画布尺寸 / DOM / 组件状态无关」。
 *
 * 纯计算契约：
 * - 本模块导出三组确定性原语：冻结的 MAP_CAMERA_CONSTRAINTS（约束不变量）、DEFAULT_CAMERA_POSE
 *   （默认机位）、clampDistance / clampPolarAngle / clampTarget / computeCameraNear（钳制与派生
 *   函数）。它们共享同一份派生尺度常量，不存在第二套魔法坐标。
 * - 钳制函数是「输入 → 合法输出」的纯函数：超界输入被确定性夹回边界，合法输入原样返回；同一输入
 *   在任意时刻产生同一输出（无隐式状态、无 DOM 读取、无随机）。OrbitControls 组件（MapOrbitControls）
 *   每帧调用 clampTarget 把 target 钳回地图包围盒、调用 computeCameraNear 让近裁剪面跟随相机高度；
 *   距离 / 极角由 OrbitControls 内部按 minDistance / maxDistance / maxPolarAngle 强制，本模块的
 *   clampDistance / clampPolarAngle 作为同一不变量的纯计算镜像，供自动化测试与（未来）入场状态机
 *   在不依赖控制器实例的情况下复算。
 *
 * 「resize 后约束仍成立」的工程语义：
 * - 约束只随 MAIN_MAP_WORLD_BOUNDS 与地形尺度常量变化，而后者是模块加载时一次性投影并冻结的常量，
 *   不感知画布尺寸；窗口 resize 改变的只是相机 aspect / 画布像素尺寸，不会影响米制约束。故钳制
 *   函数在 resize 前后对同一输入给出同一输出——这是「显式状态 / 纯计算」的直接结果，而非额外的
 *   resize 监听逻辑。
 *
 * 默认东南斜俯视机位（SPEC §2「默认视角」、§4.1）：
 * - target = 主图世界中心在地表的点 (0, 0, centerZ)：x 关于原点对称故 x=0，z 取主图南北中点
 *   centerZ，y=0（海平面，即地形未位移前的参考面）。
 * - 相机位于 target 的东南上方：相对 target 的方向单位向量由方位角（东南 45°）+ 仰角（30°）
 ...
 *   主图在东南斜俯视默认视角下完整落入画面（系数 2.1 经 TASK-006 静态机位无头渲染验证校准，
 *   并非魔法绝对米数）。
 *
 * 近裁剪面（near）动态跟随相机高度——与 TASK-007 深度精度修复的协同设计：
 * - 背景：米制世界 + Mm 级相机距离下，24 位深度缓冲精度 ≈ z²/(near·2²⁴)。TASK-007 发现海面（y=0）
 *   与沿岸低地（world-y≈0–200m）共面，near=1000m 时默认视距处精度 ≈ 数公里，透明海面会经
 *   LessEqualDepth 盖过陆地着色；修复是把静态 near 提到 0.5×半对角线（默认视距处全图精度 0.7–5.7m）。
 * - 交互冲突：固定大 near 与「允许拉近探索」矛盾——最近距离（0.3×半对角线）+ 最大极角（88°）下
 *   相机高度仅 ≈ 51.7km，任何大于该余量的固定 near 都会把整个场景裁进近裁剪面（画面全空）。
 *   能在默认远距保住米级精度的 near（≈2.5Mm）与该余量相差约 100 倍，**不存在**同时满足两端的
 *   固定 near——故 near 必须随相机高度动态伸缩：相机越高（拉远 / 俯角小）near 越大（精度越好），
 *   相机越低（拉近 / 俯角大）near 越小（不切场景）。
 * - 公式：near = (cameraY − MAX_DISPLACED_TERRAIN_Y) · NEAR_CLEARANCE_RATIO（见 computeCameraNear）。
 *   不裁切保证是**代数性**的：任意地表点 P 有 P.y ≤ MAX_DISPLACED_TERRAIN_Y，故相机 C 到 P 的距离
 *   |C−P| ≥ C.y − P.y ≥ C.y − 峰值 = 2·near > near——以相机高度减地形峰值（垂直净空）的一半为
 *   near，永远不会切到任何地表点。而「相机高度恒大于地形峰值」由约束结构性保证：
 *   cameraY = 距离·cos(极角) ≥ minDistance·cos(maxPolarAngle) > MAX_DISPLACED_TERRAIN_Y
 *   （minDistance 的地形峰值下界项专门确保此事，测试断言）。
 * - 默认机位处 near ≈ 0.52×半对角线 ≈ 2.57Mm，与 TASK-007 已验证的 0.5×半对角线几乎一致（默认
 *   视距远角精度 ≈ 5m），即默认视觉行为零回归；far 固定 8×半对角线，覆盖最不利几何（target 平移
 *   到远角 + 最大距离 ≈ 6×半对角线）仍有 33% 余量。
 */

import { MAIN_MAP_WORLD_BOUNDS } from '../lib/projection'
import { CHINA_TERRAIN_ELEVATION_ENCODING } from '../geo-contracts'
import { TERRAIN_EXAGGERATION_MAX } from '../config/terrain-config'
...
/**
 * 垂直夸张后地形的最高世界 y（米），相机「不得穿入地形」与动态 near 净空的共同基准。
 *
 * 高程编码上限 9000m（SPEC §5.1，契约层唯一源）× 最大夸张 k=3.0（SPEC §3.2，配置层唯一源）
 * = 27000m。两个唯一源任一变化（如换 DEM 编码区间或放宽 k 上限）都会经本常量自动传导到
 * minDistance 与 computeCameraNear，不存在第二份峰值常量。
 */
export const MAX_DISPLACED_TERRAIN_Y =
 ...
 * 并留适度余量；该系数经 TASK-006 静态机位（同参数）无头渲染验证校准。它是「相对地图尺度的系数」，
 ...
 * 相机垂直视场角（度）。
 *
 * 42° 在大屏常见 16:9 / 21:9 宽屏下水平视场足够覆盖主图横向跨度，同时垂直方向不会过窄导致南北
 * 边缘出画。本值同时参与默认距离与 far 的尺度协调，作为相机约束的明确配置（非魔法）。
 */
 ...
 *    MAX_DISPLACED_TERRAIN_Y，否则相机会穿入地表（同时这也是动态 near 净空 > 0 的前提，见文件头）。
 *    反推得 最近距离 ≥ 峰值 / cos(maxPolarAngle)，再乘 1.5 安全余量。cos88°≈0.035，该下界
 *    ≈ 27000/0.035·1.5 ≈ 1.16e6 m。
 ...
 * 平面（CAMERA_FAR = MAP_HALF_DIAGONAL · 8），不触发深度裁剪。距离由 OrbitControls 的
 ...
 * 相机远裁剪平面（米，固定）。
 *
 * 取 MAP_HALF_DIAGONAL · 8：覆盖最不利几何——最远距离（4·半对角线）+ target 平移到包围盒远角后
 * 相机到地图对角的距离（再 +2·半对角线）≈ 6·半对角线，仍留 33% 余量；同时不过大以保留深度缓冲
 * 有效精度（深度精度由 near 主导，far 仅需有界）。
 */
const CAMERA_FAR = MAP_HALF_DIAGONAL * 8

/**
 * 动态近裁剪面的净空比例（无量纲）。
 *
 * near = (相机高度 − 地形峰值) · 本比例。取 0.5：near 恒为垂直净空的一半，对「相机正下方恰有
 * 最高峰」这一最不利几何仍留 2 倍不裁切余量（代数证明见文件头与 computeCameraNear）；同时足够大，
 * 使默认机位处 near ≈ 0.52×半对角线，保住 TASK-007 修复的海陆深度分离精度（默认远角 ≈ 5m）。
 */
export const NEAR_CLEARANCE_RATIO = 0.5

/**
 * 由相机世界高度派生近裁剪面距离（米，纯函数）。
 *
 * near = (cameraYMeters − MAX_DISPLACED_TERRAIN_Y) · NEAR_CLEARANCE_RATIO。
 *
 * 合法定义域：cameraYMeters > MAX_DISPLACED_TERRAIN_Y（相机高于夸张后地形峰值）。该前提由相机约束
 * 结构性保证（cameraY ≥ minDistance·cos(maxPolarAngle) > 峰值，测试断言），OrbitControls 逐帧强制
 * 距离 / 极角后恒成立。MapOrbitControls 每帧以 camera.position.y 调用本函数并写回 camera.near；
 * 对定义域外输入（非有限或 ≤ 峰值，正常运行不可达）调用方跳过当帧写回——不虚构替代值。
 *
 * 不裁切保证（代数性）：对任意地表点 P（P.y ≤ 峰值），|C−P| ≥ C.y − P.y ≥ C.y − 峰值
 * = near / NEAR_CLEARANCE_RATIO = 2·near > near。即任何地表点都在近裁剪面之外至少一倍 near。
 */
export function computeCameraNear(cameraYMeters: number): number {
  return (cameraYMeters - MAX_DISPLACED_TERRAIN_Y) * NEAR_CLEARANCE_RATIO
}
 ...
 *
 * App 把 position 喂给 R3F Canvas 的 camera prop、target 由 MapOrbitControls 写入 OrbitControls
 * 实例，二者共用本常量，杜绝两处各算一遍产生漂移。
 */
 ...
 * 受约束相机的全部不变量（冻结）。
 *
 * 这是 OrbitControls 装配（MapOrbitControls）、Canvas camera prop（App）与自动化测试共享的同一份
 * 事实源：距离 / 极角由 OrbitControls 内置 minDistance / maxDistance / maxPolarAngle 强制，target
 * 边界由 MapOrbitControls 每帧调用 clampTarget 强制，near 初值由 Canvas camera prop 取
 * initialNear（挂载后每帧由 computeCameraNear 跟随相机高度）；不存在第二套约束实现。冻结防止
 * 运行时被偷偷放宽（如把 maxPolarAngle 改成 > 90° 会允许看到地底），任何调整都必须改本模块并
 * 同步测试。
 */
export const MAP_CAMERA_CONSTRAINTS: Readonly<{
  ...
  /**
   * 近裁剪面初值（米）= 默认机位高度处的动态 near（≈ 0.52×半对角线，与 TASK-007 已验证的
   * 静态 0.5×半对角线几乎一致）。Canvas camera prop 消费；挂载后由 MapOrbitControls 每帧按
   * computeCameraNear(camera.position.y) 跟随更新（见文件头「动态 near」）。
   */
  initialNear: computeCameraNear(DEFAULT_POSITION.y),
  /** 远裁剪平面（米，固定）。 */
  far: CAMERA_FAR,
})
 ...
 *
 * 这是「禁止把观察目标拖出地图有效范围」的唯一实现（SPEC §4.1）：MapOrbitControls 每帧调用本函数
 * 把 OrbitControls 平移后的 target 拉回包围盒；不存在第二套平移边界逻辑。
 */
