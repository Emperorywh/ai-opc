/**
 * heightmap 纹理加载（渲染层资产访问子层，TASK-006）。
 *
 * 角色与依赖方向：
 * - 本模块属于渲染层的资产访问子层（src/three），把 TASK-003 交付的 16 位 heightmap 资产
 *   （public/terrain/china-heightmap-4096.r16 + meta）加载、校验、解码为 GPU 纹理，供
 *   ChinaTerrainMesh 的 vertex shader 按顶点 UV 采样做位移、fragment shader 按像素 UV 重采样做
 *   分层设色。本模块**只**依赖：契约层（validateTerrainMeta / 元数据类型）、运行时高程层
 *   （decodeHeightmapBytes —— 16 位小端解码的唯一入口，避免本层另写一套解码）、three.js
 *   （DataTexture 类型）。不依赖 React / R3F / DOM 事件 / hover / GeoJSON。
 *
 * 16 位端到端精度（SPEC §5.1、§7.1）：
 * - .r16 是 16 位小端 uint16 原始字节（4096²×2B ≈ 32MB），不经任何 8 位浏览器图像解码——浏览器
 *   图像解码会把每通道压到 8 位，丢失高程精度（SPEC §5.1 红线）。本模块用 fetch 取 ArrayBuffer，
 *   再经 decodeHeightmapBytes（小端 Uint16Array 零拷贝视图）拿回 16 位编码，全程零 8 位中间态。
 * - 归一化上传：uint16 编码 → float32 归一化值（code/65535）→ RedFormat+FloatType DataTexture。
 *   float32 尾数 23 位 > 16 位，归一化值完整保留 65536 个量级，端到端无精度损失（HalfFloat 仅
 *   10 位尾数 = 1024 量级，会退化，故**不用** HalfFloat）。GPU LinearFilter 在归一化值上做硬件
 *   双线性，与 CPU 高程层「先解码四角再双线性米值」在仿射等价下一致（见 src/lib/elevation.ts）。
 *
 * 加载失败语义（绝不静默退化为平面 fallback）：
 * - 元数据不通过契约校验、字节长度与分辨率不符、fetch 失败：抛错（带稳定信息），由上层在加载期
 *   暴露，不返回「平地纹理」伪结果——平地 fallback 会把「看不到起伏」伪装成「成功」，违反
 *   SPEC §7.1 与本 TASK 验收。
 *
 * 共享 CPU 高程事实源（SPEC §3.6「运行时须将 heightmap 解码为一份 CPU 侧数组…供边界
 * densification / 遮挡射线 / 海面以下判定共用」）：
 * - 本加载器在构造 GPU 纹理时已用 decodeHeightmapBytes 把 .r16 解码为一份 Uint16Array（16 位高程
 *   的唯一 CPU 表示，约 32MB）。把它一并返回（pixels 字段），使上层能用 createElevationProvider
 *   包装出共享 ElevationProvider，供省界贴地 densify（TASK-009）、标签遮挡（TASK-010）、海面以下
 *   判定（TASK-007）等 CPU 消费者查询与 GPU 同一份高程事实源——零额外取数、零额外解码、零额外
 *   32MB（pixels 即构造纹理时已解码的那份，小端主机上是源字节缓冲的零拷贝视图）。GPU 纹理
 *   （float 归一化）与 CPU pixels（uint16）是同一源的两种表示（SPEC §3.6「GPU 位移用的纹理是
 *   另一份」指表示不同，非各自取数）。
 */
 ...
 *
 * pixels 是构造纹理时已解码的 Uint16Array（16 位高程 CPU 表示），供上层包装为共享
 * ElevationProvider，使省界贴地 / 遮挡 / 海面以下判定等 CPU 消费者与 GPU 共用同一份高程事实源
 * （零额外取数 / 解码 / 内存）。
 */
...
export async function loadHeightmapTexture(
  ...
  // pixels 同时用于构造 GPU 纹理（上面）与一并返回供上层包装共享 CPU ElevationProvider。
