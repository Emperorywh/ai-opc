 ...
 *
 * 本模块是 TASK-003「DEM heightmap 资产 pipeline」的可测试核心：
 ...
 * 已交付的生产资产（public/terrain/）由 fetch-etopo1-grid.ts 编排本核心产出，见 README §1A。
 */
...
/** Copernicus DEM 路径的默认 sourceId，供元数据 source 字段追溯（生产资产用 src-etopo1-noaa，见 fetch-etopo1-grid.ts）。 */
...
function main(): void {
  ...
  // 只读选项对象一次性组装（HeightmapBuildOptions 字段为 readonly），不写可变成员。
  const buildOptions: HeightmapBuildOptions = {
    ...(opts.width && opts.height
      ? { targetResolution: { width: Number(opts.width), height: Number(opts.height) } }
      : {}),
    ...(opts['source-id'] ? { sourceId: opts['source-id'] } : {}),
  }
  ...
  try {
    ...
    console.log(`  来源：${result.meta.source.sourceId}（详见 provenance 审计 sidecar）`)
  } catch (cause) {
