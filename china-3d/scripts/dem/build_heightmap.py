...
这是 TASK-003 的「Python 生产路径」：处理真实 GeoTIFF DEM 的下载 / 读取本地缓存、拼接、
裁剪、EPSG:4326→EPSG:3857 重投影、双线性重采样、16 位线性编码与元数据导出。
...
注意：Copernicus GLO-30 开阔海域为 0 或无效值、不含海洋水深，无法通过「海域含负高程」
地势抽样；已交付的生产资产由 fetch-etopo1-grid.ts（NOAA ETOPO1，含水深）产出，见 README §1A。
本路径保留给「纯陆地 heightmap」或将来融合正式水深源的生产场景。

字节布局（与 TS 核心严格一致）：
- .r16：行主序、行 0 = 北、列 0 = 西、每像元 2 字节小端 uint16。
- .meta.json：满足 terrain-meta 契约（src/geo-contracts/terrain.ts），可由
  pnpm verify:assets 校验。
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    p.add_argument("--source-id", default=PRODUCTION_SOURCE_ID,
                   help="元数据 sourceId（默认 src-copernicus-dem；生产资产用 src-etopo1-noaa，见 README §1A）。")
...
def main(argv: list[str]) -> int:
    ...
    print(f"  来源：{args.source_id}（详见 provenance 审计 sidecar）")
    return 0
