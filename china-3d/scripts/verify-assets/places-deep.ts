 ...
 * 调用同一函数。
 *
 * 与契约校验的关系：契约校验（validatePlaceDirectory）只验地点条目的字段结构
 ...
 *   跳过包含校验（SPEC §3.7「落入对应行政区或具有显式校正说明」）。
 *   未提供几何时跳过包含校验并在 samples 标记，避免「未检却假装通过」。
 *
 * 政治红线独立锚点（SPEC §6「台湾省与台北」「港澳齐」）：
 ...
 * 锚点与行政中心，校验随之确定性失败。九段线 / 南海岛礁 / 钓鱼岛 / 藏南 / 阿克赛钦的红线由
 * political-deep.ts 独立闭环，本模块不得越权声称已完成。
 */
...
export function verifyPlacesAsset(input: PlacesAssetVerificationInput): PlacesAssetOutcome {
  ...
  if (input.sourcesRegistry !== undefined || input.provinceDirectory !== undefined) {
    const bundleOutcome = validateContractBundle({
      sources: input.sourcesRegistry as DataSourceRegistryContract | undefined,
  ...
  // 10) 点位 - 省域几何包含。仅当提供几何时执行；未提供则在 samples 标记未检。
  ...
  if (input.provinceGeometry !== undefined) {
    ...
    for (const entry of entries) {
      const c = entry.coordinate
      if (!c || typeof c.lon !== 'number' || typeof c.lat !== 'number') continue
      // 已附显式校正说明 → 跳过包含校验（SPEC §3.7「落入对应行政区或具有显式校正说明」）。
  ...
  // 11) 审计 sidecar 完整性比对（防篡改锚点，与 terrain / provinces provenance 同构）。
