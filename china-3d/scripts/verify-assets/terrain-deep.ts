 ...
 * 被 CLI（scripts/verify-assets/run.ts 的 terrain scope）与测试基线（tests/dem/）共同复用，
 * 避免校验逻辑双轨：CLI 读盘后调用本函数，测试以篡改副本调用同一函数。
 *
 * 与契约校验的关系：契约校验（validateTerrainMeta）只验元数据 JSON 的字段结构；
 * 本模块在其之上追加「资产级」不变量——栅格字节与元数据一致、16 位精度未丢失、编码区间可还原、
 * 地势相对关系成立、provenance 审计 sidecar 的完整性摘要逐项可复算。
 *
 * 位深与米制解码依据（SPEC §5.1、§7.1）：
 ...
 *
 * 地势抽样依据（SPEC §12.1、§13；「不硬编码插值像元魔法常量」）：
 ...
 *     · 塔里木盆地区域均值显著低于其北缘天山 / 南缘昆仑区域均值（盆地凹陷，SPEC §12.1）。
 ...
 * - 塔里木盆地：40.5°N/83°E 塔克拉玛干腹地，盆地底 ~1000m。
 * - 塔里木北缘（天山）：42.5°N/83°E，均值 2500m+；南缘（昆仑）：37°N/82°E，均值 4000m+。
...
const TARIM_BASIN_REGION: Region = { centerLat: 40.5, centerLon: 83, halfSizeLat: 0.6, halfSizeLon: 1.2 }
const TARIM_NORTH_RIM_REGION: Region = { centerLat: 42.5, centerLon: 83, halfSizeLat: 0.4, halfSizeLon: 1.0 }
const TARIM_SOUTH_RIM_REGION: Region = { centerLat: 37, centerLon: 82, halfSizeLat: 0.4, halfSizeLon: 1.0 }
...
const ASSET_ERROR_CODES = {
  ...
  tarimNotLowerThanRims: 'terrain-asset.tarim-not-lower-than-rims',
...
export interface TerrainAssetSamples {
  ...
  readonly tarimBasinMeters: number
  readonly tarimNorthRimMeters: number
  readonly tarimSouthRimMeters: number
 ...
 * 区域均值而非单像元，保证抽样对重采样分辨率稳定（不硬编码插值像元魔法常量）。
 */
...
export function verifyTerrainAsset(input: TerrainAssetVerificationInput): TerrainAssetOutcome {
  ...
  // 1) 元数据契约校验：位深、CRS、编码区间、版本等结构不变量。
  ...
  // 抽样摘要默认值（元数据非法时用 NaN 占位，避免后续比较引用未初始化值）。
  let samples: TerrainAssetSamples = {
    tibetanMeters: NaN,
    easternMeters: NaN,
    sichuanBasinMeters: NaN,
    sichuanSurroundingsMeters: NaN,
    tarimBasinMeters: NaN,
    tarimNorthRimMeters: NaN,
    tarimSouthRimMeters: NaN,
  ...
  if (metaAndSizeOk) {
    ...
    const tarimBasinMeters = sampleRegionMeanMeters(validMeta, pixels, width, height, TARIM_BASIN_REGION)
    const tarimNorthRimMeters = sampleRegionMeanMeters(validMeta, pixels, width, height, TARIM_NORTH_RIM_REGION)
    const tarimSouthRimMeters = sampleRegionMeanMeters(validMeta, pixels, width, height, TARIM_SOUTH_RIM_REGION)
    ...
    // 塔里木盆地均值须显著低于北缘天山与南缘昆仑（两侧分别判定）。阈值 400m 与四川盆地一致，
    // 远低于实测差距（盆地 ~1000m，天山/昆仑 3000m+）。
    if (!(tarimNorthRimMeters > tarimBasinMeters + 400) || !(tarimSouthRimMeters > tarimBasinMeters + 400)) {
      errors.push({
        code: ASSET_ERROR_CODES.tarimNotLowerThanRims,
        path: '$.terrainSampling',
        message:
          `塔里木盆地区域均值 ${tarimBasinMeters.toFixed(0)}m 未显著低于周边山地` +
          `（北缘天山 ${tarimNorthRimMeters.toFixed(0)}m / 南缘昆仑 ${tarimSouthRimMeters.toFixed(0)}m，两侧均需高出 ≥400m）。`,
      })
    }
  ...
  if (input.provenance !== undefined && input.provenance !== null) {
    ...
    // 审计来源须与元数据来源一致（来源可审计性的交叉印证；完整来源声明见 sidecar 的 source 块）。
