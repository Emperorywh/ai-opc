 ...
 * 每个条目以 adminId 关联到契约层规范 34 省目录（CHINA_ADMINISTRATIVE_DIRECTORY，
 * 标识方案 CN-<GB/T 2260 adcode>，如 CN-440000（广东）、CN-710000（台湾）、
 * CN-810000（香港）、CN-820000（澳门））。本目录的 adminId 集合必须与规范目录精确一致——
 ...
 * 九段线 / 南海岛礁 / 钓鱼岛 / 藏南 / 阿克赛钦的国标完整性由政治边界补充资产
 * （scripts/political）独立闭环，本目录不越权声明。
 */
...
export interface PlaceCatalogProvince {
  /** 稳定行政区标识：CN-<adcode>，须命中规范 34 省目录。 */
  readonly id: string
...
/**
 * 行政区类型在此仅作文档对照（与规范 34 省目录的类型一致），不参与地点契约结构；
...
/**
 * 34 个省级行政区的地点真值，按 adcode 升序（与规范 34 省目录同序），保证同一真值表
...
export const PLACE_CATALOG: readonly PlaceCatalogEntry[] = [
  ...
  {
    id: 'CN-650000', type: 'province', shortName: '新疆', capitalName: '乌鲁木齐',
...
/**
 * 省级行政区的恰好数量（与契约层 EXPECTED_PROVINCIAL_ADMINISTRATIVE_COUNT 对齐 = 34）。
...
/**
 * 政治红线独立锚点（SPEC §6 红线；与 scripts/provinces/province-catalog.REQUIRED_POLITICAL_IDS 对齐）。
 *
