 ...
 *
 * 锚点人工校正的承载（SPEC §3.7「不得用组件内魔法偏移承载」）：
...
/**
 * 由真值目录组装完整的地点目录契约（纯函数，供 CLI 写盘与测试复用）。
 * 测试据此断言「重产输出与已交付资产逐字节一致」，证明资产确由本管线产出、未漂移。
 */
export function buildPlaceDirectoryContract(sourceId: string = DEFAULT_SOURCE_ID): PlaceDirectoryContract {
  return {
...
function main(): void {
  ...
  const directory = buildPlaceDirectoryContract(options.sourceId)
